...
define begin
  cTitle :    'Kostenstellen'
  cFile :     846
  cMenuName : 'Std.Bearbeiten'
  cPrefix :   'KSt'
  cZList :    $ZL.Kostenstellen
  cKey :      1
  cListen     : 'Kostenstellen'
end;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_KSt_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (w_Auswahlmode) or (Rechte[Rgt_KSt_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (w_Auswahlmode) or (Rechte[Rgt_KSt_Loeschen]=n);
