...
define begin
  cTitle :    'Instandhaltungsursachen'
  cFile :     824
  cMenuName : 'Std.Bearbeiten'
  cPrefix :   'IHA_Urs'
  cZList  :   $ZL.IHA.Ursachen
  cKey :      1
  cListen     : 'Instandhaltungsursachen'
end;
...
begin
  ...
  $edIHA.Urs.Nummer->WinFocusSet(true);
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Urs_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Urs_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Urs_Loeschen]=n);
