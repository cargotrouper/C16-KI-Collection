...
begin
  ...
  WHILE (vItem<>0) do begin
    ...
    form_Ele_Auf:elFMAkt1(var elAkt1, vCount);
    vStk  # vStk + "Auf.A.Stückzahl";
    vGewN # vGewN + Auf.A.NettoGewicht;
    vGewB # vGewB + Auf.A.Gewicht;
    vMatGewN # vMatGewN + Mat.Gewicht.Netto;
    vMatGewB # vMatGewB + Mat.Gewicht.Brutto;
    vM    # vM + Auf.A.Menge;
    vMP   # vMP + Auf.A.Menge.Preis;
    form_Ele_Auf:elFMAkt2(var elAkt2,vCount);
  ...
  Auf.A.Gewicht       # vGewB;
  Mat.Gewicht.Netto   # vMatGewN;
  Mat.Gewicht.Brutto  # vMatGewB;
  Auf.A.Menge         # vM;
  Auf.A.Menge.Preis   # vMP;
  Auf.A.MEH.Preis     # Auf.P.MEH.Preis;

  form_Ele_Auf:elFMAktFuss(var elAktFuss,vCount);
end;
...
// ------- POSITIONEN --------------------------------------------------------------------------
  ...
  WHILE (Erx<=_rLocked) do begin
    ...
    if (Wgr_Data:IstArt(Auf.P.Wgr.Dateinr)) then begin
      ...
      RekLink(252,404,4,_recFirst);       // Artikelcharge holen
    end
...
//    vPosStk         # "Auf.P.Stückzahl";
    ...
    // Material ausgeben
    if ((Wgr_Data:IstMix(Auf.P.Wgr.Dateinr)) or (Wgr_Data:IstMix(Auf.P.Wgr.Dateinr))) then begin
