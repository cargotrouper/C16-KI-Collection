...
define begin
  cTitle :    'Instandhaltungsmeldungen'
  cFile :     823
  cMenuName : 'Std.Bearbeiten'
  cPrefix :   'IHA_Mld'
  cZList  :   $ZL.IHA.Meldungen
  cKey :      1
  cListen     : 'Instandhaltungsmeldungen'
end;
...
begin
  ...
  $edIHA.Mld.Nummer->WinFocusSet(true);
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Mld_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Mld_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_IHA_Mld_Loeschen]=n);
