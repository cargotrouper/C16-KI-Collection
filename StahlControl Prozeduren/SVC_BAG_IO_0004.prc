...
//==== Business-Control ==================================================
//
//  Prozedur    SVC_BAG_IO_0004
//                  OHNE E_R_G
//  Zu Service: BAG_IO_NEW_FertPos
//
//  Info
//  Erstellt eine Weiterverarbeitung von einer Fertigung zu einer Position
//
...
begin
  ...
  vNode->apiSetDesc('Position der Fertigung','1');

  vNode # vApi->apiAdd('BAG.F.Fertigung',_TypeInt,true);
  vNode->apiSetDesc('Fertigungsnummer der Fertigung ','2');
...
begin
  ...
  Erx # BA1_IO_Data_SOA:InsertFertToPos(
      CnvIa(aArgs->getValue('BAG.IO.Nummer')),
      CnvIa(aArgs->getValue('BAG.F.Position')),
      CnvIa(aArgs->getValue('BAG.F.Fertigung')),
...
//  http://192.168.0.2:5060/?sender=A1386&service=BAG_IO_NEW_FertPos&bag.io.nummer=1336&bag.f.position=1&bag.f.fertigung=2&Bag.p.Position=3
  ...
  vArgs->Lib_XML:AppendNode(toUpper('bag.f.fertigung'),'1000');
