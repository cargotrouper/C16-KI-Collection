...
//
//
//  26.05.2008  AI  Erstellung der Prozedur
//
//  Subprozeduren
...
define begin
  cTitle      : 'Zahlungsarten'
  cFile       :  852
  cMenuName   : 'Std.Bearbeiten'
  cPrefix     : 'ZhA'
  cZList      : $ZL.Zahlungsarten
  cKey        : 1
  cListen     : 'Zahlungsarten'
end;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_ZhA_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_ZhA_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_ZhA_Loeschen]=n);
