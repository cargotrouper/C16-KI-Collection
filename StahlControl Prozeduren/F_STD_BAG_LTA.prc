...
begin
  ...
  if (aSeite=1) then begin
    ...
    if (Bag.P.Aktion = c_BAG_Tafel) then begin
       Pl_Print('Lohntafelauftrag'+' '+AInt(Bag.P.Nummer)+ '/' + AInt(Bag.P.Position)   ,cPos0 );
    end;
    else begin
       Pl_Print('Lohnabcoilauftrag'+' '+AInt(Bag.P.Nummer)+ '/' + AInt(Bag.P.Position)   ,cPos0 );
    end;
    ...
    if (Bag.P.Aktion = c_BAG_Tafel) then begin
       ...
       PL_Print('Bitte coilen Sie unten aufgeführte Materialien wie angegeben ab:',cPos0);
    end;
    PL_PrintLine;
    PL_PrintLine;
    // Kopftext drucken
    ...
    pls_Fontattr # _WinFontAttrBold;
    if (Bag.P.Aktion = c_BAG_Tafel) then begin
        Pl_Print('Lohntafelauftrag/Lohnabcoilauftrag '+AInt(Bag.P.Nummer)+ '/' + AInt(Bag.P.Position)   ,cPos0 );
    end;
    else begin
        Pl_Print('Lohntafelauftrag/Lohnabcoilauftrag '+AInt(Bag.P.Nummer)+ '/' + AInt(Bag.P.Position)   ,cPos0 );
    end;
  ...
  if (Form_Mode = 'FERTIGUNG') then begin
    ...
    PL_Print(   'Breitentol.' ,cF_Pos4);
    PL_Print_R( 'Länge / mm'  ,cF_Pos5);
    PL_Print(   'Längentol.  ',cF_Pos6);
    PL_Print_R( 'Vpg.'        ,cF_Pos7);
    PL_Print_R( 'Anzahl'      ,cF_Pos8);
    PL_Print_R( 'Netto kg'    ,cF_Pos9);
...
*/
    ...
    // Breitentoleranz
    PL_Print(BAG.F.Breitentol,cF_Pos4);

    // Länge
    vText #  ANum("BAG.F.Länge","Set.Stellen.Länge");
    PL_Print_R(vText,cF_Pos5);

    // Längentoleranz
    PL_Print("BAG.F.Längentol",cF_Pos6);

    // Verpackungsangabe
    if (Bag.F.Verpackung <> 0) then begin
      PL_PrintI(Bag.F.Verpackung,cF_Pos7);
      if (StrFind(vVpg,CnvAi(Bag.F.Verpackung,_FmtNumLeadZero)+';',0) = 0) then
        vVpg # vVpg + CnvAi(Bag.F.Verpackung,_FmtNumLeadZero)+';';
    end;
