...
//==== Business-Control ===================================================
//
//  Prozedur    Mobile_Data
//                  OHNE E_R_G
//  Info
//        Prozeduren zur Aufbereitung der Daten für die mobile Anwendung
//        auf dem iPad bzw. iOS.
//
//  31.05.2012  PW  Erstellung
//  04.06.2012  AI  PDF-Reports eingebunden
//
//  Subprozeduren
...
begin
  ...
  // Anschriften
  FOR  vErx # RecLink(101, 100, 12, _recNext);
  LOOP vErx # RecLink(101, 100, 12, _recNext);
  WHILE (vErx <= _rLocked) DO BEGIN
    vContact->Lib_XML:Append(CreateContactAddressXml());
  END;

  // Ansprechpartner
  FOR  vErx # RecLink(102, 100, 13, _recFirst);
  LOOP vErx # RecLink(102, 100, 13, _recNext);
  WHILE (vErx <= _rLocked) DO BEGIN
...
//=========================================================================
// MAIN
//        Bereite die XML Daten für die mobile Anwendung vor.
//        call Mobile_Data
//=========================================================================
MAIN
local begin
...
begin
  ...
  // Vertreter
  FOR  vErx # RecRead(110, 1, _recFirst);
  LOOP vErx # RecRead(110, 1, _recNext);
  WHILE (vErx <= _rLocked) DO BEGIN
    vBuf110   # RekSave(110);
    ...
    // Adressen
    FOR  vErx # RecLink(100, 110, 4, _recFirst);
    LOOP vErx # RecLink(100, 110, 4, _recNext);
    WHILE (vErx <= _rLocked ) DO BEGIN
      vBuf100  # RekSave(100);
