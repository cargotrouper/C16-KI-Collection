...
//==== Business-Control ==================================================
//
//  Prozedur    BA1_F_Schael_Main
//                    OHNE E_R_G
//  Info
//
//
//  27.07.2015  AH  Erstellung der Prozedur
//  05.04.2022  AH  ERX
...
define begin
  cDialog :   $BA1.F.Schaelen.Maske
  cTitle :    'Schälen Fertigung'
  cFile :     703
  cMenuName : 'BA1.F.Bearbeiten'
  cPrefix :   'BA1_F_Schael'
//  cZList :    0
  cKey :      1
...
//  SetStdAusFeld('edBAG.F.Artikelnummer'   ,'Struktur');
  ...
  BA1_F_Data:SumInput(BAG.F.MEH);//'kg');   28.07.2015
...
begin
  ...
  if (aName='') or (y) then begin
    ...
    BAG.F.Gewicht # Lib_Berechnungen:KG_aus_StkDBLWgrArt(1, BAG.F.Dicke, BAG.F.Breite, "BAG.IO.Länge", BAG.F.Warengruppe, "BAG.F.Güte", BAG.F.Artikelnummer);
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (BAG.F.Fertigung=999) or (BAG.P.Aktion=c_BAG_Fahr) or (BAG.P.Aktion=c_BAG_Versand) or
