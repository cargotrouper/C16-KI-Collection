...
begin
  ...
  // Art, D, B, Güte, Stk, Gew, Menge, RestStk, RestGew, MengeRest, Stk, Gew, M
    if (BAG.IO.Materialnr<>0) and (BAG.IO.Materialtyp=c_IO_Mat) then begin
      ...
      vHdl2->WinLstCellSet(BAG.IO.Ist.Out.GewB,   cClmGew, _WinLstDatLineLast);
...
begin
  ...
  if (BAG.FM.Menge<=0.0) then begin
    ...
    $NB.Main->wpcurrent # 'NB.Page1';
    $edBAG.FM.Gewicht->WinFocusSet(true);
    RETURN false;
  end;
...
begin
  ...
  if (Erx<=_rLocked) then begin
    if (BAG.F.Streifenanzahl - BAG.IO.Ist.Out.Stk-vStk<0) then begin
