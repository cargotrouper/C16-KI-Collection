...
//    SUB HoleEmpfaenger();
//    SUB Druck_Chem(aName : alpha; aName2 : alpha; pMin : float; pMax : float);
//    SUB Druck_Mech(Name : alpha; pMin : float; pMax : float; Einheit : alpha;);
//    SUB Druck_Artikel(aRb1 : alpha; aWMenge : float; aPMenge : float; aStk : int;);
//    SUB Druck_Material(aRb1 : alpha; aWMenge : float; aPMenge : float; aStk : int;);
//    SUB Druck_Material_Lohn(aRb1 : alpha; aWMenge : float; aPMenge : float; aStk : int;);
//    SUB Druck_BAG();
...
define begin
  ...
  cPosKopf1 : 120.0
  cPosKopf2 : 155.0
  cPosKopf3 : 40.0  // Feld Lieferanschrift
...
//            Wird nur benötigt für Druck_Material()
//            Argumente:
...
begin
  ...
  if (pMin<>0.0 or pMax<>0.0) then begin
    ...
    if (pMin<>0.0 and pMax<>0.0) then
      PL_Print(ANum(pMin,-1) + ' - ' + ANum(pMax,-1),CnvFI(GV.Int.01*30)+cpos2a+5.0)
    if (pMin<>0.0 and pMax=0.0) then
      PL_Print('min. ' + ANum(pMin,-1),CnvFI(GV.Int.01*30)+cpos2a+5.0)
    if (pMin=0.0 and pMax<>0.0) then
      PL_Print('max. ' + ANum(pMax,-1),CnvFI(GV.Int.01*30)+cpos2a+5.0)
...
begin
  ...
  if(pMin<>0.0 or pMax<>0.0)then begin
    ...
    if(pMin<>0.0 and pMax<>0.0)then begin
      PL_Print(ANum(pMin,-1) + Einheit + ' - ' + ANum(pMax,-1) + Einheit,75.0);end;
    if(pMin<>0.0)then begin
      if (pMax=0.0) then
        PL_Print(Name,cpos2a);PL_Print('min. ' + ANum(pMin,-1) + Einheit,75.0);
    end;
    if(pMin=0.0)then begin
      if (pMax<>0.0) then
        PL_Print(Name,cpos2a);PL_Print('max. ' + ANum(pMax,-1) + Einheit,75.0);
    end;
    if(pMin=pMax)then
      PL_Print(Name,cpos2a);PL_Print(ANum(pMin,-1) + Einheit,75.0);
    PL_PrintLine;
  end;
end;
...
//========================================================================
//  Druck_Artikel
//            Druckt die Artikeldaten für ein Druckformular
//            Wird benötigt in allen Druckroutinen
//========================================================================
sub Druck_Artikel(
  aRb1      : alpha;
  aWMenge   : float;
...
***/
  ...
  if (Auf.P.AusfOben <> '') or (Auf.P.AusfUnten <> '') then begin
    ...
    if (Auf.P.AusfOben <> '') then begin
      ...
      WHILE (Erx <= _rLocked) DO BEGIN
        if(Auf.AF.Seite = '1') then begin
           if(vVerp = '') then
             vVerp # 'Vorderseite: ' + Auf.AF.Bezeichnung;
           else
            vVerp # vVerp + ', ' + Auf.AF.Bezeichnung
        end;
    ...
    if (Auf.P.AusfUnten <> '') then begin
      Erx # RecLink(402, 401, 11, _recFirst);
      WHILE (Erx <= _rLocked) DO BEGIN
        if(Auf.AF.Seite = '2') then begin
          if(vMerker = '') then
            vMerker # Auf.AF.Bezeichnung;
          else
            vMerker  # vMerker  + ', ' + Auf.AF.Bezeichnung
        end;
        Erx # RecLink(402, 401, 11, _recNext);
      END;
      if (vVerp <> '') then
  ...
  if ((Auf.P.RID = 0.0) AND (Auf.P.RIDMAX <> 0.0)) then begin
    PL_Print('RID ' + Auf.AbmessungsEH + ':',cpos2);
    PL_Print('max. ' + ANum(Auf.P.RIDMAX,-1),cPos2a);
    PL_PrintLine;
  end;
  ...
  if ((Auf.P.RAD <> 0.0) AND (Auf.P.RADMAX <> 0.0)) then begin
    PL_Print('RAD ' + Auf.AbmessungsEH + ':',cpos2);
    PL_Print(ANum(Auf.P.RAD,-1) + ' - ' + ANum(Auf.P.RADMAX,-1)  ,cPos2a);
    PL_PrintLine;
  end else
  if ((Auf.P.RAD = 0.0) AND (Auf.P.RADMAX <> 0.0)) then begin
    PL_Print('RAD ' + Auf.AbmessungsEH + ':',cpos2);
    PL_Print('max. ' + ANum(Auf.P.RADMAX,-1),cPos2a);
    PL_PrintLine;
  end;
  ...
  if (Auf.P.KgmmVon + Auf.P.KgmmBis  <> 0.0) then begin
    if (Auf.P.KgmmVon <> 0.0 and Auf.P.KgmmBis <> 0.0) then
      vMerker # 'Kg/mm: min. ' + ANum(Auf.P.KgmmVon,-1) + ' max. ' + ANum(Auf.P.KgmmBis,-1)
    if (Auf.P.KgmmVon <> 0.0 and Auf.P.KgmmBis = 0.0) then
      vMerker # 'Kg/mm: min. ' + ANum(Auf.P.KgmmVon,-1)
    if (Auf.P.KgmmVon = 0.0 and Auf.P.KgmmBis <> 0.0) then
      vMerker # 'Kg/mm: max. ' + ANum(Auf.P.KgmmBis,-1)
    if (Auf.P.KgmmVon = Auf.P.KgmmBis) then
      vMerker # 'Kg/mm: ' + ANum(Auf.P.KgmmBis,-1)
    ADD_VERP(vMerker,'')
  end;
  ...
  if (Auf.P.RechtwinkMax > 0.0) then
    ADD_VERP('max. Rechtwinkligkeit: ', ANum(Auf.P.RechtwinkMax,-1));

  if (Auf.P.EbenheitMax > 0.0) then
    ADD_VERP('max. Ebenheit: ', ANum(Auf.P.EbenheitMax,-1));

  if ("Auf.P.SäbeligkeitMax" > 0.0) then
    ADD_VERP('max. Säbeligkeit: ', ANum("Auf.P.SäbeligkeitMax",-1));
...
//Lib_PrintLine:PrintPic(cPos5,cPos5+20.0,20.0,'*Z:\c16\client.53\bilder\skizzen\skizze100.jpg');
  ...
  Druck_Mech('Streckgrenze',Auf.P.Streckgrenze1 , Auf.P.Streckgrenze2,'N/mm²');
  Druck_Mech('Zugfestigkeit',Auf.P.Zugfestigkeit1 , Auf.P.Zugfestigkeit2,'N/mm²');
  ...
  Druck_Mech('Rp 0,2',Auf.P.DehngrenzeA1 , Auf.P.DehngrenzeA2,'N/mm²');
  Druck_Mech('Rp 10',Auf.P.DehngrenzeB1 , Auf.P.DehngrenzeB2,'N/mm²');
  if ("Set.Mech.Titel.Körn" <> '') then Druck_Mech("Set.Mech.Titel.Körn","Auf.P.Körnung1", "Auf.P.Körnung2",'')
  else Druck_Mech('Körnung',"Auf.P.Körnung1", "Auf.P.Körnung2",'');
  if ("Set.Mech.Titel.Härte" <> '') then Druck_Mech("Set.Mech.Titel.Härte","Auf.P.Härte1" , "Auf.P.Härte2",'')
  else Druck_Mech('Härte',"Auf.P.Härte1" , "Auf.P.Härte2",'');
  // Sonstiges
  ...
  Druck_Chem('C' ,Set.Chemie.Titel.C   ,Auf.P.Chemie.C1,Auf.P.Chemie.C2);
  Druck_Chem('Si',Set.Chemie.Titel.Si  ,Auf.P.Chemie.Si1,Auf.P.Chemie.Si2);
  Druck_Chem('Mn',Set.Chemie.Titel.Mn  ,Auf.P.Chemie.Mn1,Auf.P.Chemie.Mn2);
  Druck_Chem('P' ,Set.Chemie.Titel.P   ,Auf.P.Chemie.P1,Auf.P.Chemie.P2);
  Druck_Chem('S' ,Set.Chemie.Titel.S   ,Auf.P.Chemie.S1,Auf.P.Chemie.S2);
  Druck_Chem('Al',Set.Chemie.Titel.Al  ,Auf.P.Chemie.Al1,Auf.P.Chemie.Al2);
  Druck_Chem('Cr',Set.Chemie.Titel.Cr  ,Auf.P.Chemie.Cr1,Auf.P.Chemie.Cr2);
  Druck_Chem('V' ,Set.Chemie.Titel.V   ,Auf.P.Chemie.V1,Auf.P.Chemie.V2);
  Druck_Chem('Nb',Set.Chemie.Titel.Nb  ,Auf.P.Chemie.Nb1,Auf.P.Chemie.Nb2);
  Druck_Chem('Ti',Set.Chemie.Titel.Ti  ,Auf.P.Chemie.Ti1,Auf.P.Chemie.Ti2);
  Druck_Chem('N' ,Set.Chemie.Titel.N   ,Auf.P.Chemie.N1,Auf.P.Chemie.N2);
  Druck_Chem('Cu',Set.Chemie.Titel.Cu  ,Auf.P.Chemie.Cu1,Auf.P.Chemie.Cu2);
  Druck_Chem('Ni',Set.Chemie.Titel.Ni  ,Auf.P.Chemie.Ni1,Auf.P.Chemie.Ni2);
  Druck_Chem('Mo',Set.Chemie.Titel.Mo  ,Auf.P.Chemie.Mo1,Auf.P.Chemie.Mo2);
  Druck_Chem('B' ,Set.Chemie.Titel.B   ,Auf.P.Chemie.B1,Auf.P.Chemie.B2);
  Druck_Chem(''  ,Set.Chemie.Titel.1   ,Auf.P.Chemie.Frei1.1,Auf.P.Chemie.Frei1.2);
  PL_PrintLine;
...
//  PL_PrintI_L(Auf.Nummer,cPosKopf2);
  ...
  if(vBuf100Re -> Adr.Steuernummer <> '') then begin
    pls_fontSize # 9;
  ...
  if (Auf.Vorgangstyp = c_AUF) then
    Pl_Print('Auftragsbestätigung'+' '+CnvAi(Auf.P.Nummer)+'     '+Frm.Markierung ,cPos0);
  if (Auf.Vorgangstyp = c_ANG) then
    Pl_Print('Angebot'+' '+CnvAi(Auf.P.Nummer)+'     '+Frm.Markierung   ,cPos0);
  */
  if (Auf.Vorgangstyp = c_AUF) then begin
    if ("Auf.LiefervertragYN") and (!"Auf.AbrufYN") then begin
      PL_Print('Rahmenvertrag-VK ' + AInt("Auf.P.Nummer")+'     '+Frm.Markierung /*+ '      vom ' + cnvAD(Auf.Datum,_FmtInternal)*/, cPos0);
    end
    else if ("Auf.AbrufYN") and (!"Auf.LiefervertragYN") then begin
      PL_Print('Auftragsbestätigung ' + AInt("Auf.P.Nummer") /*+ '      vom ' + cnvAD(Auf.Datum,_FmtInternal)*/, cPos0);
    end
    else
      Pl_Print('Auftragsbestätigung' + ' ' + AInt(Auf.P.Nummer) /*+ '      vom ' + cnvAD(Auf.Datum,_FmtInternal)*/ ,cPos0 );
    end
  else if (Auf.Vorgangstyp = c_ANG) then
    Pl_Print('Angebot' + ' ' + AInt(Auf.P.Nummer) /*+ '      vom ' + cnvAD(Auf.Datum,_FmtInternal)*/ ,cPos0 );
    pl_PrintLine;
    if ("Auf.AbrufYN") and (!"Auf.LiefervertragYN") then begin
    pls_fontSize # 9;
    PL_Print('Abruf aus Rahmenvertrag ' + AInt("Auf.P.AbrufAufNr"), cPos0);
    PL_PrintLine;
  end;
  ...
  if (aSeite=1) then begin
    ...
    If (Auf.Vorgangstyp = c_ANG) then begin
      PL_Print('Wir danken für Ihre Anfrage, die wir zu unseren Allgemeinen Verkaufsbedingungen',cPos0);
      PL_PrintLine;
    End else begin
      PL_Print('Wir danken für Ihren Auftrag, den wir zu unseren Allgemeinen Verkaufsbedingungen',cPos0);
      PL_PrintLine;
    End;

    PL_Print('(Stand 1.1.2010) gebucht haben. Anderen Bedingungen widersprechen wir ausdrücklich.',cPos0);
    PL_PrintLine;
    PL_PrintLine;
      ...
      WHILE (Erx<=_rLocked) do begin
        ...
        ((Auf.Z.MEH='%') or (Auf.Z.MEH=Auf.P.MEH.Preis)) then begin
          if (vRb1='') then begin
            ...
            PL_Print_R(Auf.Z.Bezeichnung + ' ' + ANum(-Auf.Z.Menge,"Set.Stellen.Menge") + ' %',cPos5);
            PL_PrintLine;
    ...
    */
        ...
        if (Auf.Z.MengenbezugYN) and (Auf.Z.MEH<>'%') and (Auf.Z.Position=0) AND (Auf.Z.Nummer = Auf.Nummer)then begin
          ...
          if (Auf.Z.MEH<>Auf.P.MEH.Preis) then begin
            if (Auf.Z.MEH='m') or (Auf.Z.MEH='qm') then
              PL_PrintF(vMenge,2,cPos3a)
            else if (Auf.Z.MEH='t') then
              PL_PrintF(vMenge,3,cPos3a)
            else
...
//          vMwstWert1 # vMwstWert1 + vPreis;
    ...
    'Summe' : begin
      ...
      pls_Fontsize # 9;
      PL_Print_R(ANum(vMwstSatz1,1) + '% MwSt. '+ "Wae.Kürzel",cPos7-25.0);
      PL_PrintF(vMwstWert1,2,cPos7);
      PL_PrintLine;

      if (vMwstSatz2>0.0) then begin
        pls_Fontsize # 9;
        PL_Print_R(ANum(vMwstSatz2,1) + '% MwSt. '+ "Wae.Kürzel",cPos7-25.0);
    ...
    'BAG-Verpackungsposition': begin
      ...
      if (BAG.Vpg.KgmmVon + BAG.Vpg.KgmmBis  <> 0.0) then begin
        if (BAG.Vpg.KgmmVon <> 0.0 and BAG.Vpg.KgmmBis <> 0.0) then
          vMerker # 'Kg/mm: min. ' + ANum(BAG.Vpg.KgmmVon,-1) + ' max. ' + ANum(BAG.Vpg.KgmmBis,-1)
        if (BAG.Vpg.KgmmVon <> 0.0 and BAG.Vpg.KgmmBis = 0.0) then
          vMerker # 'Kg/mm: min. ' + ANum(BAG.Vpg.KgmmVon,-1)
        if (BAG.Vpg.KgmmVon = 0.0 and BAG.Vpg.KgmmBis <> 0.0) then
          vMerker # 'Kg/mm: max. ' + ANum(BAG.Vpg.KgmmBis,-1)
        if (BAG.Vpg.KgmmVon = BAG.Vpg.KgmmBis) then
          vMerker # 'Kg/mm: ' + ANum(BAG.Vpg.KgmmBis,-1)
        ADD_VERP(vMerker,'')
      end;
      ...
      if (BAG.Vpg.RechtwinkMax > 0.0) then
        ADD_VERP('max. Rechtwinkligkeit: ', ANum(BAG.Vpg.RechtwinkMax,-1));

      if (BAG.Vpg.EbenheitMax > 0.0) then
        ADD_VERP('max. Ebenheit: ', ANum(BAG.Vpg.EbenheitMax,-1));

      if ("BAG.Vpg.SäbeligMax" > 0.0) then
        ADD_VERP('max. Säbeligkeit: ', ANum("BAG.Vpg.SäbeligMax",-1));
...
//  Lib_Print:FrmJobOpen(CnvAi(vNummer,_FmtNumNogroup),vHeader , vFooter,y,y,n);
  // <<< MUSTER
  if (Lib_Print:FrmJobOpen(true, vHeader ,vFooter, true, true, false) < 0) then begin
...
//  DMS_ArcFlow:SetDokName('!SC\Verkauf','AB',Auf.Nummer);
  ...
  if (Auf.Vorgangstyp = c_ANG) then
    DMS_ArcFlow:SetDokName('ANG',Auf.Nummer, Form_DokAdr)
  else
    DMS_ArcFlow:SetDokName('AB',Auf.Nummer, Form_DokAdr);
...
// ------- POSITIONEN --------------------------------------------------------------------------
  ...
  WHILE (RecLink(401,400,9,vFlag) <= _rLocked ) DO BEGIN
    ...
    else if ((Wgr_Data:IstMix(Auf.P.Wgr.Dateinr)=false) and (Wgr_Data:IstMat(Auf.P.Wgr.Dateinr)=false)) then CYCLE;
    ...
    // Artikel ausgeben
    if (Wgr_Data:IstArt(Auf.P.Wgr.Dateinr)) then begin
      Druck_Artikel(vRb1, Auf.P.Menge.Wunsch, vPosMenge, vPosStk);
    end;
    ...
    if (Wgr_Data:IstMix(Auf.P.Wgr.Dateinr)) or (Wgr_Data:IstMat(Auf.P.Wgr.Dateinr)) then begin
      if (AAr.Berechnungsart>=700) then
        Druck_Material_Lohn(vRb1, Auf.P.Menge.Wunsch, vPosMenge, vPosStk)
      else begin
        vPosStk         # "Auf.P.Stückzahl";
        Druck_Material(vRb1, Auf.P.Menge.Wunsch, vPosMenge, vPosStk);
      end;
...
//aFilename # 'D:\debug\aaa.pdf';
//fsidelete(aFilename);
//Lib_Print:FrmJobClosePDF(y, aFilename, false);
//debugx('');
  // Objekte entladen
  if (vPL<>0) then PL_Destroy(vPL);
