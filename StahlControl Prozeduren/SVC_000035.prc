...
//
//  Info
///  AAR_SEL: Lesen von Auftragsarten und Rückgabe der angegeben Felder
//
...
*/
 ...
 // Haupdaten
  addToApi('AAr.Nummer','');
  addToApi('AAr.Bezeichnung','');
  addToApi('AAr.Berechnungsart','');
  addToApi('AAr.KonsiYN','');
  addToApi('AAr.ReserviereSLYN','');
  addToApi('AAr.ReservierePosYN','');
  addToApi('AAr.Lohn.Aktion','');
  addToApi('AAr.Lohn.Res.Gruppe','');
  addToApi('AAr.Lohn.Ressource','');
...
begin
  ...
  vArgs # VarAllocate(Sel_Args_000035);
...
*/
  ...
  // --------------------------------------------------------------------------
  // Daten selektieren
  vSel  # Lib_SOA:CreatePartSel(c_DATEI, 1, 'SVC_000035:Sel',vArgs);
    ...
    case (toUpper(vFelderGrp)) of
      ...
      'INDI' : begin
        ...
        WHILE (vArgNode > 0) do begin
          ...
          if (StrCut(vFldName,1,4) = 'AAR.') then begin
...
begin
  ...
  VarInstance(Sel_Args_000035, SOA_PartSel_Args);
