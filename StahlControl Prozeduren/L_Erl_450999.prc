...
//===== Business-Control =================================================
//
//  Prozedur    L_Erl_450999 / temporär
//                    OHNE E_R_G
//  Info        Umsatzauswertung mit verdichtet
//
//
//  13.03.2008  ST  Erstellung der Prozedur
//  28.07.2008  DS  QUERY
//  20.02.2013  TM  Kopie aus 450.010
//  13.06.2022  AH  ERX
//
//  Subprozeduren
//    SUB AusSel();
...
begin
  ...
  gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Sel.LST.450999',here+':AusSel'); // Temporär, muss dringend geändert werden!
...
//      vMarge # Auf.A.RechPreisW1 - vEKEff;    // Absolute Marge
      ...
      //   DO BEGIN
      //     List_Spacing[i+17]  # List_Spacing[i+16] + 20.0;
      //     Write(i+16, ZahlF(ga_KstVal[i],2)        ,y , _LF_Wae, 3.0);
    ...
    'Summe' : begin
      ...
      if (List_XML) then begin
        ...
        DO BEGIN
          List_Spacing[i+17]  # List_Spacing[i+16] + 20.0;
          Write(i+16, ZahlF(ga_KstValSum[i],2)        ,y , _LF_Wae, 3.0);
        END;
      end;  // XML-Ausgabe
  ...
  if (List_XML) then begin
    ...
    DO BEGIN
      ...
      if (RecRead(846,1,0) <= _rLocked) then
        Write(i+16, Kst.Bezeichnung ,y , 0, 3.0);
    END;
  end;
...
begin
  ...
  // Selektion öffnen
  //vSelName # Sel_Build(vSel, 404, 'LST.450999',y,0);
