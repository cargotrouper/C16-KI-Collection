...
//==== Business-Control ==================================================
//
//  Prozedur    BA1_F_Saegen_Main
//                    OHNE E_R_G
//  Info
//
//
//  24.10.2011  AI  Erstellung der Prozedur
//  05.04.2022  AH  ERX
//  19.08.2022  HA  Quick Jump
...
define begin
  cDialog :   $BA1.F.Saegen.Maske
  cTitle :    'Sägen-Fertigung'
  cFile :     703
  cMenuName : 'BA1.F.Bearbeiten'
  cPrefix :   'BA1_F_Saegen'
  cKey :      1
end;
...
begin
  ...
  if (aName='') or (y) then begin
    ...
    if (Abs("BAG.IO.Länge")>9999.99) then begin
      ...
      $Lb.Laenge.E->wpcaption   # ANum("BAG.IO.Länge"/1000.0,3);
      end
    else begin
    ...
    if (Abs("BAG.IO.Länge"-vL)>9999.99) then begin
      ...
      $Lb.Laenge.R->wpcaption   # ANum(("BAG.IO.Länge"-vL)/1000.0, 3);
      end
    else begin
  ...
  if (aName='') or (aName='edBAG.F.Kommission') then begin
    ...
    if (BAG.F.AuftragsNummer<>0) then begin
      ...
      if ("Auf.P.Stückzahl"-Auf.P.Prd.Plan.Stk>0) then begin
        $Lb.Meter.A->wpcaption    # ANum("Auf.P.Länge" * Cnvfi(("Auf.P.Stückzahl"-Auf.P.Prd.Plan.Stk)) ,2);
        end
      else begin
  ...
  if ((vS=_WinTypeEdit) or (vS=_WinTypeFloatEdit) or (vS=_WinTypeIntEdit)) then
    ...
    case (aEvt:Obj->wpname) of
      ...
      'edBAG.F.Laenge': begin
        ...
        $edBAG.F.Streifenanzahl->winupdate(_WinUpdFld2Obj);
        RefreshIfm();
      end;
...
begin
  ...
  vAnz # 0;
  if (BAG.IO.Plan.Out.Stk<>0) then
    vAnz # BAG.F.Streifenanzahl;//"BAG.F.Stückzahl" div BAG.IO.Plan.Out.Stk;

  aL # aL + ("BAG.F.Länge" * cnvfi(vAnz));
  aGew # aGew + BAG.F.Gewicht;
  aM # aM + BAG.F.Menge;

  FOR vI # 1 loop inc (vI) while (vI<=18) do begin
    vHdl # Winsearch(gMdi,'bar.E'+cnvai(vBar));
    vBar # vBar + 1;
    if (vHdl<>0) then begin
      if (vI<=vAnz) then begin
        vHdl->wparealeft  # vBarPos;
        vJ # cnvif("BAG.F.Länge" * vBarFak);
        vHdl->wparearight # vBarPos + vJ;
        vHdl->wpcaption # ANum("BAG.F.Länge","Set.Stellen.Länge");
