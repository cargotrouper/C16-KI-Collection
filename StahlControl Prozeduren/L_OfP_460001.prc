...
//===== Business-Control =================================================
//
//  Prozedur    L_OfP_460001
//                    OHNE E_R_G
//  Info        Offene Posten Gesamtliste
//
//
//  02.08.2010  AI  Erstellung der Prozedur
//  14.04.2014  TM  Kundennummer als Sortierkriterium hinzugefügt
//  13.06.2022  AH  ERX
//  2023-06-19  SR  Datum auf
//
//  Subprozeduren
//    SUB AusSel();
...
begin
  ...
  Sel.Adr.von.KdNr          # 0;
  Sel.von.Datum             # 0.0.0;
  Sel.bis.Datum             # 31.12.2099; //SR Proj. 2469/84: vorher auf today gestellt, aber soll wohl nicht viel Sinn ergeben
  Sel.von.Datum2            # 0.0.0;
  Sel.bis.Datum2            # 31.12.2099; //SR Proj. 2469/84: vorher auf today gestellt, aber soll wohl nicht viel Sinn ergeben
  "Sel.Fin.!GelöschteYN"    # y;
  "Sel.Fin.GelöschteYN"     # n;
  Sel.Fin.nurMarkeYN        # n;
  gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Sel.LST.460001',here + ':AusSel');
...
begin
  case aName of
    ...
    'HEADER' : begin
      ...
      List_Spacing[11] # List_Spacing[10] + 15.0; // ü.Tage
      List_Spacing[12] # List_Spacing[11] + 29.0; // ReBetrag
      List_Spacing[13] # List_Spacing[12] + 29.0; // offen
      ...
      LF_Set(11,'Re.Betrag ' + "Set.Hauswährung.Kurz"  ,y , 0);
      LF_Set(12, 'off.Betrag ' + "Set.Hauswährung.Kurz" ,y , 0);
      LF_Set(13,'Vertr.'                               ,y , 0);
      LF_Set(14,'Stufe'                                ,y , 0);
      if(List_XML = true) then
        LF_Set(15,'Bemerkung'                                ,y , 0);
    end;
    ...
    'POSTEN' : begin
      if (aPrint) then begin
        ...
        //if (OfP.Zieldatum <> 00.00.00) then
      ...
      LF_Set(5, '#IntLimit'                   ,y , _LF_WAE);
      LF_Set(6, '@OfP.Rechnungsnr'            ,y , _LF_INT);
      LF_Set(7, '#Rechungsdatum'              ,y , _LF_Date);
      LF_Set(8, '#TageSeitReDat'              ,y , _LF_Int);
      LF_Set(9,'#Zieldatum'                   ,y , _LF_Date);
      LF_Set(10,'#Tagedif'                    ,y , _LF_Int);
      LF_Set(11,'@OfP.BruttoW1'               ,y , _LF_WAE);
      LF_Set(12,'@OfP.RestW1'                 ,y , _LF_WAE);
      LF_Set(13,'@OfP.Vertreter'              ,y , _LF_INT);
      LF_Set(14,'@OfP.Mahnstufe'              ,y , _LF_INT);
      if(List_XML = true) then
        LF_Set(15,'@OfP.Bemerkung'            ,n , 0);
    end;
...
begin
  ...
  if (aSort=4) then vKey # 2;   // Kundennummer
  ...
  If (Sel.Fin.nurMarkeYN) then begin
    ...
    if ("Sel.Fin.GelöschteYN") then begin
      ...
      if (Erx <> 0) then
        Lib_Sel:QError(vSel);
      erx # vSel->SelDefQuery('KundeAbl', vQ100);
      if (Erx <> 0) then
        Lib_Sel:QError(vSel);
      vSelName # Lib_Sel:SaveRun(var vSel, 0);
  ...
  WHILE (vItem != 0) do begin
    ...
    if (aSort=3 or aSort =4) and (vKu<>OfP.Kundennummer) and (vKu<>-1) then begin
  ...
  if (aSort=3 or aSort =4) and (vI>1) and (GetSum(3)<>0.0) then begin
    LF_Print(g_Summe1);
  end;
