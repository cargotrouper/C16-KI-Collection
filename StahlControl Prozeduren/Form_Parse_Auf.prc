...
begin
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse400(var aLabels, var aInhalt, var aZusatz,aText,aKombi,aBuf100Re,aBuf101We,aBuf110Ver1,aBuf110Ver2);
    ...
    'ABTITEL'       :       begin
                              if (Auf.Vorgangstyp = c_AUF) then begin
                                if ("Auf.LiefervertragYN") and (!"Auf.AbrufYN") then
                                  ...
                                  vA # 'Auftragsbestätigung ' + AInt("Auf.P.Nummer")+' aus Rahmenvertrag ' + AInt("Auf.P.AbrufAufNr")
                                else
                                  vA # 'Auftragsbestätigung' + ' ' + AInt(Auf.P.Nummer);
                                end
                              else if (Auf.Vorgangstyp = c_ANG) then begin
                                vA # 'Angebot' + ' ' + AInt(Auf.P.Nummer);
     ...
     'RECHTITEL2' :         begin
                              if (Auf.Vorgangstyp=c_BOGUT) then begin
                                ...
                                if ("Auf.P.AbrufAufNr"<>0) then
                                  vA # vA + ' zu Rechnung ' + AInt("Auf.P.AbrufAufNr")
                                end
                              else if (Auf.Vorgangstyp=c_GUT) then begin
                                vA # 'Gutschrift ' + AInt(Erl.Rechnungsnr);
                                end
                              else if (Auf.Vorgangstyp=c_BEL_KD) then begin
                                vA # 'Belastung ' + AInt(Erl.Rechnungsnr);
                                if ("Auf.P.AbrufAufNr"<>0) then
                                  vA # vA + ' zu Rechnung ' + AInt("Auf.P.AbrufAufNr")
                                end
                                ...
                                vA # 'Rechnung ' + AInt(Erl.Rechnungsnr);
                              if (vA<>'') and (Frm.Markierung<>'') then
                                vA # vA +'     '+Frm.Markierung;
                              vAdd # vA;
                            end;
    'TITELZUSATZ' :         begin
                              if ("Auf.GültigkeitVom"<>0.0.0) or ("Auf.GültigkeitBis"<>0.0.0) then begin
                                ...
                                if ("Auf.GültigkeitVom"<>0.0.0) then
                                  vA3 # vA3 + ' ab '+cnvad("Auf.GültigkeitVom");
                                if ("Auf.GültigkeitBis"<>0.0.0) then
                                  vA3 # vA3 + ' bis '+cnvad("Auf.GültigkeitBis");
                                vA3 # vA3 + ')';
                              end;
...
//                          AddA('(Stand 1.1.2010) gebucht haben. Anderen Bedingungen widersprechen wir ausdrücklich.', var aLabels);
                                ...
                                if (vA2<>'') then vA2 # ' durch '+vA2;
                                if (Auf.Best.Nummer<>'') then vA # ' '+Auf.Best.Nummer;
                                vA # vA + vA2;
                                if (Auf.Best.Datum<>0.0.0) then vA2 # ' vom '+cnvad(Auf.Best.Datum);
                                vA # vA + vA2;
                                vA # 'Ihre Bestellung'+vA+' bestätigen wir wie folgt:'+vA3;
                              end;
                              vAdd # vA;
                            end;
    ...
    'WARENEMPFAENGER' :  begin
        ...
        //                              StrAdj(aBuf101We->Adr.A.LKZ,_StrBegin | _StrEnd)    + '-' +
                            ...
                            // if (Adr.A.Warenannahme1) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Warenannahme1,_StrBegin | _StrEnd);
                            // if (Adr.A.Warenannahme2) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Warenannahme2,_StrBegin | _StrEnd);
                            // if (Adr.A.Warenannahme3) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Warenannahme3,_StrBegin | _StrEnd);
                            // if (Adr.A.Warenannahme4) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Warenannahme4,_StrBegin | _StrEnd);
                            // if (Adr.A.Warenannahme5) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Warenannahme5,_StrBegin | _StrEnd);
                            // if (Adr.A.Betriebsferien) !='' then
                            //   vA # vA +StrChar(10)+ StrAdj(aBuf101We->Adr.A.Betriebsferien,_StrBegin | _StrEnd);
                        end;
    ...
    'LIB.BEZEICHNUNG'  :  vAdd # Lib.Bezeichnung.L1;
    'VSA.BEZEICHNUNG' :   vAdd # VsA.Bezeichnung.L1;
    'ZAB.BEZEICHNUNG' :   begin
                            vA # ZaB.Bezeichnung1.L1;
                            if (ZaB.Bezeichnung2.L1<>'') then vA # vA + ' ' +ZaB.Bezeichnung2.L1;
...
begin
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse400Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi,aBuf100Re,aBuf101We,aBuf110Ver1,aBuf110Ver2);
    RETURN(vZeilen);
  end;
...
begin
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse401(var aLabels, var aInhalt, var aZusatz,aText,aKombi);
    RETURN(vZeilen);
  end;
  // Sonderbefehl?
  if (StrCut(aText,1,2)='my') then begin
  ...
  case (StrCnv(vFeld, _StrUpper)) of
    ...
    'MENGE+MEH' :   if ("Auf.P.Menge.Wunsch"<>0.0) then begin
                      if (Auf.P.MEH.Wunsch='kg') then
                        ...
                        vAdd # anum("Auf.P.Menge.Wunsch", 0)+' '+Auf.P.MEH.Wunsch
                      else
    ...
    'MENGE2+MEH' :   if ("Auf.P.Menge"<>0.0) then begin
                      if (Auf.P.MEH.Einsatz='kg') then
                        ...
                        vAdd # anum("Auf.P.Menge", 0)+' '+Auf.P.MEH.Einsatz
                      else
...
// je @Breite
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse401Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi);
    RETURN(vZeilen);
  end;
...
begin
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse403(var aLabels, var aInhalt, var aZusatz,aText,aKombi,aGesamtpreis);
    RETURN(vZeilen);
  end;
  // Sonderbefehl?
  if (StrCut(aText,1,2)='my') then begin
  ...
  case (StrCnv(vFeld, _StrUpper)) of
    'MENGE+MEH' :   if ("Auf.Z.Menge"<>0.0) then begin
                      ...
                      if (Auf.Z.MEH='Stk') then
                        vAdd # anum("Auf.Z.Menge", 0)+' '+Auf.Z.MEH
    ...
    'BEZEICHNUNG'  : vAdd # Auf.Z.Bezeichnung;
    'MEH' :         begin
...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse403Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi,aGesamtpreis);
    RETURN(vZeilen);
  end;
...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse404(var aLabels, var aInhalt, var aZusatz,aText,aKombi,aCount);
    RETURN(vZeilen);
  end;
  ...
  case (StrCnv(vFeld, _StrUpper)) of
    ...
    'WGRBEZEICHNUNG' :  vAdd # Wgr.Bezeichnung.L1;
...
begin
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Auf_E:Parse404Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi,aCount);
    RETURN(vZeilen);
  end;
