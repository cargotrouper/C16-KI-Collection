...
define begin
  cTitle :    'Reklamationsarten'
  cFile :     849
  cMenuName : 'Rek.Art.Bearbeiten'
  cPrefix :   'Rek_Art'
  cZList :    $ZL.RekArten
  cListen   : 'Reklamationsarten'
  cKey :      1
end;
...
begin
  ...
  $edRekArt.Nummer->winfocusset(true);
end;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_RekArt_Anlegen]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_RekArt_Aendern]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_RekArt_Loeschen]=n);
  // Menü sperren
