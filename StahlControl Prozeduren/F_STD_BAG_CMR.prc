...
// ------- EINSATZMATERIAL -----------------------------------------------------------------
  ...
  WHILE (Erx <= _rLocked) DO BEGIN
    ...
    if (BAG.IO.Materialtyp = c_IO_VSB) then begin
      ...
      if(Mat.Coilnummer != '') then begin
          PL_Print('Coil Nr.:' + Mat.Coilnummer,cPos2);
      end;

      if(Mat.Werksnummer != '') then begin
    ...
    if (BAG.IO.Materialtyp = c_IO_Mat) then begin
      ...
      PL_Print_R(cnvAF(Mat.Gewicht.Brutto,_FmtNumNoZero,0, Set.Stellen.Gewicht),cPos8);
      PL_Print_R(cnvAF(Mat.Gewicht.Netto,_FmtNumNoZero,0,Set.Stellen.Gewicht),cPos9);
      PL_Printline;
