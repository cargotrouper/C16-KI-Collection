...
//  30.05.2012  ST  Jumplogic eingebaut
//  31.05.2012  AI  Set.Mat.Lagerasnchrift eingebaut
//  08.08.2012  AI  Status nur noch über Menü änderbar (Projekt 1381/93)
//  01.10.2012  AI  VSBMeldung-Datum eingebaut
//  10.10.2012  AI  Rechte:Excelexport
//  19.10.2012  AI  Neu: "Mnu.Creete.Auftrag"
//  25.02.2013  ST  Vererben2 hinzugefügt
//  02.04.2013  AI  Kg/mm-Berechnung in RefreshIfm deaktiviert (wird ja beim Speichern gemacht)
//  06.06.2013  AI  StatusSetzen fragt Grund ab
//  18.09.2013  ST  Info Alibidaten hinzugefügt (1326/368)
...
//  15.10.2015  ST  Rechte für Filter eingebaut Start/Stop Projekt 1577/4
...
//  13.07.2020  AH  Analyseprüfung verbessert
//  01.09.2020  AH  Kurzinfo
//  29.09.2020  AH  "Set.Mat.LyseCheck"
//  18.11.2020  ST  Neu: Materialkartendruck für markierte Karten
//  2022-06-28  AH  ERX
//
//  Subprozeduren
...
begin
  ...
  // Underline setzen...    MUSTER
  Lib_Guicom2:Underline($edMat.Warengruppe);
  Lib_Guicom2:Underline($edMat.Guetenstufe);
  Lib_Guicom2:Underline($edMat.Guete);
  Lib_Guicom2:Underline($edMat.LieferStichwort);
  Lib_Guicom2:Underline($edMat.LagerStichwort);
  Lib_Guicom2:Underline($edMat.Status);
  Lib_Guicom2:Underline($edMat.Intrastatnr);
  Lib_Guicom2:Underline($edMat.Strukturnr);
  Lib_Guicom2:Underline($edMat.Zeugnisart);
  
  Lib_Guicom2:Underline($edMat.Lageradresse);
  Lib_Guicom2:Underline($edMat.Lageranschrift);
  Lib_Guicom2:Underline($edMat.Lagerplatz);
  Lib_Guicom2:Underline($edMat.Lieferant);
  Lib_Guicom2:Underline($edMat.Erzeuger);
  Lib_Guicom2:Underline($edMat.Ursprungsland);
  
  Lib_Guicom2:Underline($edMat.Analysenr);
  Lib_Guicom2:Underline($edMat.Verwiegungsart);
  Lib_Guicom2:Underline($edMat.Etikettentyp);
  Lib_Guicom2:Underline($edMat.Zwischenlage);
  Lib_Guicom2:Underline($edMat.Unterlage);
  Lib_Guicom2:Underline($edMat.Umverpackung);
  ...
  if (Set.LyseErweitertYN) then begin
    vHdl # Winsearch(aEvt:Obj, 'lbMat.SbeligkeitMax');
    if (vHdl<>0) then vHdl->wpVisible # false;
    vHdl # Winsearch(aEvt:Obj, 'edMat.SbeligkeitMax');
    if (vHdl<>0) then vHdl->wpVisible # false;
    vHdl # Winsearch(aEvt:Obj, 'lbMat.SaebelProM');
    if (vHdl<>0) then vHdl->wpVisible # false;
    vHdl # Winsearch(aEvt:Obj, 'edMat.SaebelProM');
    if (vHdl<>0) then vHdl->wpVisible # false;
    vHdl # Winsearch(aEvt:Obj, 'lbSaebel');
    if (vHdl<>0) then vHdl->wpVisible # false;

    SetStdAusFeld('edMat.Analysenr2'      ,'Analyse2');

    vPar # Winsearch(aEvt:Obj, 'NB.Page3');

    Lib_GuiCom2:Hide(vPar, 'lbMat.Mech.Sonstiges1', 'edMat.Chemie.Frei1.1');
    Lib_GuiCom2:Hide(vPar, 'lbMat.Mech.Sonstiges2', 'edMat.Chemie.Frei1.2');
    Lib_GuiCom2:Hide(vPar, 'lbMat.Analysenr');

    vNext # Winsearch(vPar, 'edMat.Chemie.Frei1.2');

    vHdl  # Winsearch(aEvt:Obj, 'edMat.Analysenr');
    Lib_GuiCom:ObjSetPos(vHdl, 144, 84);
    vHdl # Lib_GuiCom2:CreateObjFrom(vHdl, _WinTypeIntEdit, vPar, 'edMat.Analysenr2', '', _WinJustRight, 144, 110, 378 ,25, vNext);
    vHdl->wpDbFieldName # 'Mat.Analysenummer2';

    vHdl # Winsearch(aEvt:Obj, 'bt.Analyse');
    Lib_GuiCom:ObjSetPos(vHdl, 256, 84);
    vHdl # Lib_GuiCom2:CreateObjFrom(vHdl, _WinTypeButton, vPar, 'bt.Analyse2', '', _WinJustCenter, 256, 22, 378 ,25, vNext);

    vHdl # Winsearch(vPar, 'bt.AnalyseAnzeigen1');
    Lib_GuiCom:ObjSetPos(vHdl, 280, 84);

    vHdl # Lib_GuiCom2:CreateObjFrom(vHdl, _WinTypeButton, vPar, 'bt.AnalyseAnzeigen2', 'Analyse anzeigen', _WinJustCenter, 280, 166, 378 ,25);
    vHdl->wpCustom  # '_I';
  end;
...
begin
  ...
  vA # MQU_Data:BildeVorgabe(aName, 200, "Mat.Güte", "Mat.Dicke", var vVon, var vBis);
...
//    ((aName='edMat.Bestellt.Gew') and ($edMat.Bestellt.Gew->wpchanged)) or
  ...
  if (aName='edMat.Guete') and ($edMat.Guete->wpchanged) then begin
    ...
    Mat.Dichte      # Wgr_Data:GetDichte(Mat.Warengruppe, 200);
    $Lb.Mat.Dichte->wpcaption # ANum(Mat.Dichte,5);
    $edMat.Guete->Winupdate();
  end;

//  if (aName='') or (aName='edMat.EK.Preis') then begin
...
***/
  ...
  if (Set.Mat.LyseCheck='1') and (Mat_Data:HatAnalyse(1)=false) then begin
    ...
    CheckAnalyse2(aName, $lbMat.Streckgrenze1, 'Streckgrenze', $edMat.Streckgrenze1, $edMat.StreckgrenzeB1);
    CheckAnalyse2(aName, $lbMat.Zugfestigkeit1, 'Zugfestigkeit', $edMat.Zugfestigkeit1, $edMat.ZugfestigkeitB1);
    CheckAnalyse2(aName, $lbMat.DehnungA1, 'DehnungA', $edMat.DehnungA1);
    CheckAnalyse2(aName, $lbMat.DehnungA1, 'DehnungB', $edMat.DehnungB1, $edMat.DehnungC1);
    CheckAnalyse2(aName, $lbMat.DehnungsgrenzeA1, 'DehngrenzeA', $edMat.DehnungsgrenzeA1, $edMat.RP02_B1);
    CheckAnalyse2(aName, $lbMat.DehnungsgrenzeB1, 'DehngrenzeB', $edMat.DehnungsgrenzeB1, $edMat.RP10_B1);
    CheckAnalyse2(aName, $lbMat.Krnung1, 'Koernung', $edMat.Krnung1, $edMat.KrnungB1);
    CheckAnalyse2(aName, $lbMat.HrteA1, 'Haerte', $edMat.HaerteA1, $edMat.HaerteB1);
    CheckAnalyse2(aName, $lbMat.RauigkeitA1, 'RauigkeitA', $edMat.RauigkeitA1, $edMat.RauigkeitB1);
    CheckAnalyse2(aName, $lbMat.RauigkeitC1, 'RauigkeitB', $edMat.RauigkeitC1, $edMat.RauigkeitD1);
...
***/
  ...
  if (Set.Mat.LyseCheck<>'') and (Mat_Data:HatAnalyse(2)=false) then begin
    ...
    CheckAnalyse2(aName, $lbMat.Streckgrenze2, 'Streckgrenze', $edMat.Streckgrenze2, $edMat.StreckgrenzeB2);
    CheckAnalyse2(aName, $lbMat.Zugfestigkeit2, 'Zugfestigkeit', $edMat.Zugfestigkeit2, $edMat.ZugfestigkeitB2);
    CheckAnalyse2(aName, $lbMat.DehnungA2, 'DehnungA', $edMat.DehnungA2);
    CheckAnalyse2(aName, $lbMat.DehnungA2, 'DehnungB', $edMat.DehnungB2, $edMat.DehnungC2);
    CheckAnalyse2(aName, $lbMat.DehnungsgrenzeA2, 'DehngrenzeA', $edMat.DehnungsgrenzeA2, $edMat.RP02_B2);
    CheckAnalyse2(aName, $lbMat.DehungsgrenzeB2, 'DehngrenzeB', $edMat.DehnungsgrenzeB2, $edMat.RP10_B2);
    CheckAnalyse2(aName, $lbMat.Krnung2, 'Koernung', $edMat.Krnung2, $edMat.KrnungB2);
    CheckAnalyse2(aName, $lbMat.HrteA2, 'Haerte', $edMat.HaerteA2, $edMat.HaerteB2);
    CheckAnalyse2(aName, $lbMat.RauigkeitA2, 'RauigkeitA', $edMat.RauigkeitA2, $edMat.RauigkeitB2);
    CheckAnalyse2(aName, $lbMat.RauigkeitC2, 'RauigkeitB', $edMat.RauigkeitC2, $edMat.RauigkeitD2);
...
*/
  ...
  if (Mode=c_ModeNew) then begin
    If (Mat.Bestand.Stk=0) and (Mat.Bestellt.Stk=0) then begin
      Msg(001200,Translate('Stückzahl'),0,0,0);
      $NB.Main->wpcurrent # 'NB.Page1';
      $edMat.Bestand.Stk->WinFocusSet(true);
      RETURN false;
    end;
    If (Mat.Bestand.Gew=0.0) and (Mat.Bestellt.Gew=0.0) then begin
...
begin
  ...
  if (vFocus='jump') then begin
    ...
    case (aEvt:Obj->wpcustom) of
      ...
      'AnalyseStart' : begin
//        if ($edMat.Mech.Sonstiges1->wpvisible) then
  //        $edMat.Mech.Sonstiges1->winfocusset(false);
        Lib_GuiCom2:JumpPageObj('NB.Page3');
      end;
      'vorAnalyse' : begin
//        $edMat.Mech.Sonstiges1->winfocusset(false);
//        $NB.Main->wpcurrent # 'NB.Page2';
//        $edMat.Ursprungsland->winfocusset(false);
        Lib_GuiCom2:JumpPageObj('NB.Page2', -1);
      end;
      'nachAnalyse' : begin
//        $edMat.Chemie.Frei1.2->winfocusset(false);
//        $NB.Main->wpcurrent # 'NB.Page4';
//        $edMat.Verwiegungsart->winfocusset(false);
        Lib_GuiCom2:JumpPageObj('NB.Page4');
      end;
...
/**
if (aEvt:obj->wpName='edMat.LieferStichwort') and (aEvt:Obj->wpChanged) then begin
      Adr.LieferantenNr # cnvia(aEvt:Obj->wpCaption);
      if (Adr.LieferantenNr<>0) then begin
        RecRead(100,3,0);
        Mat.Lieferant # Adr.Lieferantennr;
        Mat.LieferStichwort # Adr.Stichwort;
        aEvt:Obj->wpcaption # Adr.Stichwort;
        Lib_GuiCom:Pflichtfeld($edMat.LieferStichwort);
      end;
      RETURN true;
    end;
**/
  // 2022-11-28 AH voher NUR bei MODENEW
...
//debugx(anum($edMat.EK.Preis->wpCaptionFloat,2)+' zu '+anum(Mat.EK.Preis,2)+' '+mat.Meh);
  ...
  if (aEvt:Obj->wpname='edMat.Status') then begin
    if (Mat.Kommission='') and ((Mat.Status=c_Status_VSB) or (Mat.Status=c_Status_VsbPuffer) or
      (Mat.Status=c_Status_VsbRahmen) or (Mat.Status=c_Status_VSBKonsi) or (Mat.Status=c_Status_VSBKonsiRahmen)) then begin
      Msg(200005,'',_WinIcoError,0,0);
      Mat.Status # 0;
      RETURN false;
    end;
...
begin
  ...
  case aBereich of
    ...
    'Analyse' : begin
      RecBufClear(230);
      Lys.K.Analysenr # Mat.Analysenummer;
    ...
    'Analyse2' : begin
      RecBufClear(230);
      Lys.K.Analysenr # Mat.Analysenummer2;
      if (Set.LyseErweitertYN) then
        gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Lys.K.Verwaltung2',here+':AusAnalyse2')
      else
        gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Lys.K.Verwaltung',here+':AusAnalyse2');
      Lib_GuiCom:RunChildWindow(gMDI);
    end;
...
begin
  if (gSelected<>0) then begin
    ...
    gSelected # 0;
    // Feldübernahme
    Mat.Analysenummer2 # Lys.K.Analysenr;
    vTmp # WinFocusget();   // LastFocus-Feld refreshen
    if (vTmp<>0) then vTmp->Winupdate(_WinUpdFld2Obj);
  end;
  // Focus setzen:
  vTmp # WinSearch(gMDI, 'edMat.Analysenr2');
...
//  $bt.Reserviert->wpdisabled # (Mode<>c_ModeView);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # ((Mode<>c_ModeList) and (Mode<>c_ModeView)) or
                      ((Mat.Status>c_Status_BisFrei) and (Mat.Status<>c_status_VSB) and (Mat.Status<>c_Status_VsbPuffer) and (Mat.Status<>c_Status_VsbRahmen) and (Mat.Status<>c_Status_VsbKonsiRahmen)) or
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # Rechte[Rgt_Mat_SerienEdit]=false;
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # Rechte[Rgt_Mat_Filter]=false;
...
***/
    ...
    'Mnu.Ktx.Errechnen' : begin
      if (aEvt:Obj->wpname='edMat.RAD') then begin
        Erx # RecLink(819,200,1,0); // Warengruppe holen
        Mat.RAD # Lib_Berechnungen:RAD_aus_KgStkBDichteRID(Mat.Bestand.Gew, Mat.Bestand.Stk, Mat.Breite, Wgr_Data:GetDichte(Wgr.Nummer, 200), Mat.RID);
        $edMat.RAD->winupdate(_WinUpdFld2Obj);
      end;
    end;
...
//      gZLList->WinUpdate(_WinUpdOn, _WinLstRecFromRecID | _WinLstRecDoSelect)
    ...
    'Mnu.Alibidaten' : begin
      Mat_Subs:ZeigeAlibidaten();
      RETURN true;
    end;
    ...
    'Mnu.Kommission' : begin
      ...
      if (vI=0) then begin
        ...
        else if (Mat.Status<>c_Status_VsbPuffer) and (Mat.Status<>c_Status_VSB) and (Mat.Status<>c_Status_VsbPuffer) and (Mat.Status<>c_Status_VsbRahmen) and (Mat.Status<>c_Status_VsbKonsiRahmen) and
          (Mat.Status<>c_Status_VSBKonsi) and (Mat.Status<>c_Status_EKVSB) and (Mat.Kommission<>'') then begin
          Msg(200027,aint(Mat.Nummer),0,0,0);
          RETURN true;
        end;
      ...
      // Markierte?
        ...
        WHILE (vMarked > 0) DO BEGIN
          ...
          if (vMFile=200) then begin
            ...
            else if (Mat.Status<>c_Status_VSB) and (Mat.Status<>c_Status_VsbPuffer) and (Mat.Status<>c_Status_VsbRahmen) and (Mat.Status<>c_Status_VsbKonsiRahmen) and
              (Mat.Status<>c_Status_VSBKonsi) and (Mat.Status<>c_Status_EKVSB) and (Mat.Kommission<>'') then begin
              Msg(200027,aint(Mat.Nummer),0,0,0);
               RekRestore(vBuf200);
              RETURN true;
...
begin
  ...
  if (aEvt:Obj->wpName='bt.AnalyseAnzeigen1') then begin
    if (Mat.Analysenummer<>0) then begin
      ...
      w_NoList # y;
      w_Command  # 'NEWREPOS'
      w_Cmd_Para # aInt(RecInfo(230,_recId));
      Lib_GuiCom:RunChildWindow(gMDI);
      RETURN true;
    end;

    if (Set.LyseErweitertYN) then begin
      if (Mat.Status=c_Status_Bestellt) then begin
        if (Ein_Data:Read(Mat.Einkaufsnr, Mat.EinkaufsPos, n)>=500) then begin
          // von Bestellung
          RecBufClear(231);
          "Lys.Trägerdatei"     # 501;
          "Lys.Trägernummer1"   # Mat.Einkaufsnr;
          "Lys.Trägernummer2"   # Mat.EinkaufsPos;
          "Lys.Trägernummer3"   # 0;
          Erx # RecRead(231,1,0);
          if (Erx<=_rLocked) then begin
            Lys_Msk_Main:Start('', false, 'Bestellung '+aint(Ein.P.Nummer)+'/'+aint(Ein.P.Position)+' '+Ein.P.LieferantenSW, "Ein.P.Güte", "Ein.P.Gütenstufe", Ein.P.Dicke);
            RETURN true;
          end;
        end;
      end;
  ...
  if (aEvt:Obj->wpName='bt.AnalyseAnzeigen2') then begin
    if (Mat.Analysenummer2<>0) then begin
      Lys.K.AnalyseNr # Mat.AnalyseNummer2;
      RecRead(230,1,0);
      if (Set.LyseErweitertYN) then
        gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Lys.K.Verwaltung2','')
      else
        gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Lys.K.Verwaltung','');
      VarInstance(WindowBonus,cnvIA(gMDI->wpcustom));
      Mode # c_modeBald + c_modeView;
...
//$clmMat.Dicke->wpFontAttr # _WinFontAttrNormal;
  ...
  if (aMark) then begin
    ...
    if (RunAFX('Mat.EvtLstDataInit','n')<0) then RETURN;
  end;
/***/
  ...
  if (Mat.LfENr=-1) then
    Gv.Alpha.03 # Translate('fehlt')
  else if (Mat.LfENr>0) then
    Gv.Alpha.03 # aint(Mat.LfeNr)
  else
    Gv.Alpha.03 # Translate('ohne');
end;
...
//    Lib_GUiCom:ObjSetPos($lb.Mat.Info2, 0, vRect:bottom+8+ (vSize / 5) + 4);
    Lib_GUiCom:ObjSetPos($lb.Mat.Info1, 0, aRect:bottom-aRect:Top-28-w_QBHeight - (vSize * 2));
    Lib_GUiCom:ObjSetPos($lb.Mat.Info2, 0, aRect:bottom-aRect:Top-28-w_QBHeight - (vSize * 1));
...
begin
  ...
  if (gSelected<>0) then begin
    ...
    Mat.A.Aktionsnr     # Mat.Sta.Nummer;
    Mat.A.Aktionsdatum  # today;
    Mat.A.Bemerkung     # vGrund;
...
begin
  ...
  if (aName = StrCnv('clmMat.Nummer',_StrUpper) AND (aBuf->Mat.Nummer<>0)) then begin
    Lib_Workbench:OpenKurzInfo(200, RecInfo(aBuf,_recID));//, 'Material '+aint(aBuf->Mat.Nummer));
    RETURN;
  end;
  
  // MUSTER
  if ((aName =^ 'edMat.Warengruppe') AND (aBuf->Mat.Warengruppe<>0)) then begin
    RekLink(819,200,1,0);   // Warengruppe holen
    Lib_Guicom2:JumpToWindow('Wgr.Verwaltung');
    RETURN;
  end;
  
//      SFX_Ein_P_Main:Start(0, Ein.P.Nummer, Ein.P.Position, true, 'NB.Page2');
//    gMDI # Lib_GuiCom:AddChildWindow(gMDI,'Wgr.Verwaltung','',y);
//    VarInstance(WindowBonus,cnvIA(gMDI->wpcustom));
//      vQ # '';
//      Lib_Sel:QInt(var vQ, '"Wgr.Dateinummer"', '=', 200);
//      Lib_Sel:QInt(var vQ, '"Wgr.Dateinummer"', '=', 209, 'OR');
...
//    RETURN;
  ...
  if ((aName =^ 'edMat.Guetenstufe') AND (aBuf->"Mat.Gütenstufe"<>'')) then begin
    MQu.S.Stufe # "Mat.Gütenstufe";
    RecRead(848,1,0);
  ...
  if ((aName =^ 'edMat.Guete') AND (aBuf->"Mat.Güte"<>'')) then begin
    "MQu.Güte1" # "Mat.Güte";
    RecRead(832,2,0);
  ...
  if ((aName =^ 'edMat.LieferStichwort') AND (aBuf->Mat.LieferStichwort<>'')) then begin
    Adr.Stichwort # Mat.LieferStichwort;
    RecRead(100,4,0);
    Lib_Guicom2:JumpToWindow('Adr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.LagerStichwort') AND (aBuf->Mat.LagerStichwort<>'')) then begin
    Adr.Stichwort # Mat.LagerStichwort;
    RecRead(100,4,0);
    Lib_Guicom2:JumpToWindow('Adr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Status') AND (aBuf->Mat.Status<>0)) then begin
    RekLink(820,200,9,0);   // Status holen
    Lib_Guicom2:JumpToWindow('Mst.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Intrastatnr') AND (aBuf->Mat.Intrastatnr<>'')) then begin
  ...
  if ((aName =^ 'edMat.Strukturnr') AND (aBuf->Mat.Strukturnr<>'')) then begin
    RekLink(250,200,26,0);   // Strukturnr holen
    Lib_Guicom2:JumpToWindow('Art.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Zeugnisart') AND (aBuf->Mat.Zeugnisart<>'')) then begin
    Zeu.Bezeichnung # Mat.Zeugnisart;
    RecRead(839,2,0);
    Lib_Guicom2:JumpToWindow('Zeu.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Lageradresse') AND (aBuf->Mat.Lageradresse<>0)) then begin
    RekLink(100,200,5,0);   // Lageradresse holen
    Lib_Guicom2:JumpToWindow('Adr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Lageranschrift') AND (aBuf->Mat.Lageranschrift<>0)) then begin
    RekLink(101,200,6,0);   // Lageranschrift holen
    Adr.A.Adressnr # Mat.Lageradresse;
    Adr.A.Nummer # Mat.Lageranschrift;
    RecRead(101,1,0);
    Lib_Sel:QInt(var vQ, 'Adr.A.Adressnr', '=', Mat.Lageradresse);
    Lib_Guicom2:JumpToWindow('Adr.A.Verwaltung',vQ);
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Lagerplatz') AND (aBuf->Mat.Lagerplatz<>'')) then begin
    LPl.Lagerplatz # Mat.Lagerplatz;
    RecRead(844,1,0);
    Lib_Guicom2:JumpToWindow('LPl.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Lieferant') AND (aBuf->Mat.Lieferant<>0)) then begin
    RekLink(100,200,4,0);   // Lieferant holen
    Lib_Guicom2:JumpToWindow('Adr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Erzeuger') AND (aBuf->Mat.Erzeuger<>0)) then begin
    RekLink(100,200,3,0);   // Erzeuger holen
    Lib_Guicom2:JumpToWindow('Adr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Ursprungsland') AND (aBuf->Mat.Ursprungsland<>'')) then begin
    RekLink(812,200,2,0);   // Ursprungsland holen
    Lib_Guicom2:JumpToWindow('Wgr.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Analysenr') AND (aBuf->Mat.Analysenummer<>0)) then begin
    RekLink(230,200,21,0);   // Analysenr holen
    Lib_Guicom2:JumpToWindow('Lys.K.Verwaltung2');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Verwiegungsart') AND (aBuf->Mat.Verwiegungsart<>0)) then begin
    RekLink(818,200,10,0);   // Verweigungsart holen
    Lib_Guicom2:JumpToWindow('VWa.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Etikettentyp') AND (aBuf->Mat.Etikettentyp<>0)) then begin
    RekLink(840,200,33,0);   // Etikettentyp holen
    Lib_Guicom2:JumpToWindow('Eti.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Zwischenlage') AND (aBuf->Mat.Zwischenlage<>'')) then begin
    ULa.Bezeichnung # Mat.Zwischenlage;
    RecRead(838,2,0);
    Lib_Guicom2:JumpToWindow('ULa.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Unterlage') AND (aBuf->Mat.Unterlage<>'')) then begin
    ULa.Bezeichnung # Mat.Unterlage;
    RecRead(838,2,0);
    Lib_Guicom2:JumpToWindow('ULa.Verwaltung');
    RETURN;
  end;
  
  if ((aName =^ 'edMat.Umverpackung') AND (aBuf->Mat.Umverpackung<>'')) then begin
    ULa.Bezeichnung # Mat.Umverpackung;
    RecRead(838,2,0);
    Lib_Guicom2:JumpToWindow('ULa.Verwaltung');
    RETURN;
  end;
