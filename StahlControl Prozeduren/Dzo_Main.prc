...
//
//
//  17.04.0023  ST  Erstellung der Prozedur
//
//  Subprozeduren
...
begin
  ...
  if (Mode=c_ModeEdit) then begin
    ...
    Dzo.Anlage.Datum  # Today;
    Dzo.Anlage.Zeit   # Now;
    Dzo.Anlage.User   # gUserName;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Dzo_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Dzo_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Dzo_Loeschen]=n);
...
begin
  ...
  case (aMenuItem->wpName) of
    ...
    'Mnu.Protokoll' : begin
      PtD_Main:View(gFile,Dzo.Anlage.Datum, Dzo.Anlage.Zeit, Dzo.Anlage.User);
    end;
