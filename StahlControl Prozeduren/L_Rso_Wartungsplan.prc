...
begin
  ...
  vFile     # 160;
  vKey      # 1;
  vName     # 'Wartungsplan für:'+AInt(Rso.Nummer)+' '+Rso.Bezeichnung1;
  vQuer     # false;
  vMyPos    # 'Lst.Rso.WartungsplanPos';
  vMyKopf   # 'Lst.Rso.WartungsplanKopf';
  ...
  If (vMahnTree = 0) then RETURN;

  // Alle Einträge der Datei lesen
  WHILE (RecLink(165,160,1,vFlags) < _rnokey) DO BEGIN
    // Nächster Datensatz
    vFlags # _RecNext;
    if (Rso.IHA.WartungYN=n) then begin
      CYCLE;
    end;
    vMahnSortKey # CnvAD(Rso.IHA.Termin);
    Sort_ItemAdd(vMahnTree,vMahnSortKey,165,RecInfo(165,_RecId));
  END;
  ...
  if (vJob > 0) then begin
    ...
    if (vPage > 0) then begin
      ...
      WHILE (vMahnItem != 0) do begin
        RecRead(165,0,_RecId,vMahnItem->spId);
        ...
        Erx # RecLink(823,165,2,0); // Meldung holen
        if (Erx>_rlocked) then RecBufClear(823);
