...
//    sub SaveDEF_Strict(aDatei : int; aName : alpha; aPrefix : alpha; aText : int; aNameSpace : alpha) : logic
...
//  WriteLN('namespace AbcNet.BusinessLogic.'+aNameSpace);
  ...
  WHILE (vI<=TextInfo(aText,_TextLines)) do begin
    ...
    if (StrCut(vA,1,5)='F_ALT') then begin
      ...
      if (aByODBC) then begin
        vFName # StrCut(Str_Token(vA,'|',1),6,20);
        vFName # Lib_strings:Strings_ReplaceAll(vFName, '.', '_');
        vFName # Lib_strings:Strings_ReplaceAll(vFName, '-', '_');
      end;
    end;
...
begin
  ...
  vFile # FSIOpen(cPath+'\'+aNameSpace+'\'+aName+'.cs', _FsiAcsRW | _FsiCreate | _FsiTruncate);
//  vFile # FSIOpen(cPath+'\Core\Data\'+aNameSpace+'\'+aName+'.cs', _FsiAcsRW | _FsiCreate | _FsiTruncate);
  if (vFile<=0) then begin
    RETURN false;
  end;
  ...
  vName     # Filename(aDatei);
//  vNewName # aName;
  vNewName # Lib_strings:Strings_ReplaceAll(vName, '.', '_');
  TextAddLine(aText, 'T_ALT'+aint(aDatei)+'|'+vNewName+'|'+aName);
  ...
  WriteLN('using System;');
  WriteLN('namespace AbcNet.Core.Data');
//  WriteLN('namespace AbcNet.Server.BusinessLogic.'+aNamespace);
  //WriteLN('namespace AbcNet.Server.BO');//+aNameSpace);
  WriteLN('{');
  ...
  FOR vI # 1 loop inc(vI) while (vI<=vJ) do begin
    ...
    if (vA<>'') then vA # vA + ', ';
    vB # FldName(aDatei, vTds, vFld);
    // Alle Punkte erseten für ODBC
    vFldName # Lib_strings:Strings_ReplaceAll(vB, '.', '_');
    vFldName # Lib_strings:Strings_ReplaceAll(vFldName, '-', '_');
    vA # vA + vFldName;
    vKeyA # vKeyA + aint(vFld)+'|';
  END;
  WriteLN(cTab+'[DbPrimaryKey("'+vA+'")]');
...
myDebug('bitte prüfen:'+aint(aDatei)+' '+vA);
      ...
      // Alle Punkte erseten für ODBC
      vFldName # Lib_strings:Strings_ReplaceAll(vA, '.', '_');
      vFldName # Lib_strings:Strings_ReplaceAll(vFldName, '-', '_');
      if (StrFind(vFldName,'%',0)>0) or (StrFind(vFldName,'!',0)>0) or (StrFind(vFldName,'$',0)>0) then
myDebug('komischer Feldname bei:'+vA);
      ...
      case FldInfo(aDatei,vTds,vFld,_FldType) of
        ...
        _TypeDate   : vTyp # 'DateTime';
        _Typeword   : vTyp # 'int';         // int16
        _typeint    : vTyp # 'int';
        _Typefloat  : vTyp # 'double';      // float??
        _typelogic  : vTyp # 'byte';        // bool
        _TypeTime   : vTyp # 'DateTime';    // Timespan
        otherwise vTyp # 'ERROR';
      end;
...
TextAddLine(aText, 'F_ALT'+vA+'|'+vB+'|'+vTyp+'|File:'+aint(aDatei));
      ...
      WriteLN(cTab+cTab+'[DbColumn("'+vFldName+'")]');
...
/***
  ...
  REPEAT
    ...
    vA # Str_Token(vA,'|',1);
    vA # StrCut(vA,6,30);
    vA # Lib_strings:Strings_ReplaceAll(vA, '.', '_');
    vA # Lib_strings:Strings_ReplaceAll(vA, '-', '_');
    if (vFirst) then begin
      vFirst # n;
      Write(vA);
...
WriteERROR('Datei '+aint(aDatei)+': FEHLENDES FELD: '+vFName);
    ...
    vA # aName + '.' + vNewFName;
    if (abyODBC) then begin
      vA # Lib_strings:Strings_ReplaceAll(vFName, '.', '_');
      vA # Lib_strings:Strings_ReplaceAll(vA, '-', '_');
      vA # aName + '.' + vA;
    end;
...
//  if (abyODBC) then begin
//    WriteLN(cTab+cTab+'public static List<'+aName+'> GetRandom(int aAnz = 10)');
//    WriteLN(cTab+cTab+'{');
//    WriteLN(cTab+cTab+cTab+'return RandomUtil.CreateList<'+aName+'>(aAnz);');
//    WriteLN(cTab+cTab+'}');
//    WriteLN('');
//    WriteLN('');
//  end;
...
//myDebug('MULTILINK '+aint(aDatei)+' '+cnvai(vIndex,0,0,2)+':'+vB);
      ...
      if (true) or (aByODBC) then begin
// 13.02.2013 keine 1 zu n        WriteLN(cTab+cTab+'[DbIgnore]');
//        WriteLN(cTab+cTab+'public List<'+vObj2Name+'> '+vB+'Obj{ get { return Get'+vB+'(null); } }');
      end;
...
WriteERROR('Datei '+aint(aDatei)+': FEHLENDES FELD: '+v2FName);
      ...
      if (aByODBC) then begin
        vA # Lib_strings:Strings_ReplaceAll(v2FNAme, '.', '_');
        vA # Lib_strings:Strings_ReplaceAll(vA, '-', '_');
        vA # vObj2Name + '.' + vA;
      end;
...
begin
  ...
  vByODBC # WindialogBox(0,'Umwandlung','für ODBC?',_WinIcoQuestion,_WinDialogYesNo,1)=_winidyes;
//vDotNET # false;
  ...
  FOR vI # 1 loop inc(vI) while (vI<=TextInfo(vTxt,_TextLines)) do begin
    ...
    if (vByODBC=false) then
      SaveDEF(vDatei, vName, vPrefix, vProto, vNameSpace)
    else
      SaveDEF_Strict(vDatei, vName, vPrefix, vProto, vNameSpace);
  END;

  // Fields-File schreiben...
  SaveFIELDS(vProto, vByODBC);
  ...
  FOR vI # 1 loop inc(vI) while (vI<=TextInfo(vTxt,_TextLines)) do begin
    ...
    SaveQUERY(vDatei, vName, vPrefix, vProto, vNameSpace, vTxt, vByODBC);
  END;


  if (vByODBC=false) then begin
/***
