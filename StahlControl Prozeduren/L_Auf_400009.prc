...
//  21.04.2015  ST  Bugfix: Gruppierung per Auftragskopf, Sortierung nach Liefertermin (frühster Pos. Termin wird gedruckt)
//  13.06.2022  AH  ERX
//
//  Subprozeduren
//    SUB AusSel();
...
begin
  ...
  WHILE (Erx <= _rLocked) DO BEGIN
    ...
    vSortkey # Lib_Strings:IntForSort(Auf.P.Nummer);

   if (aSort=1) then   vSortKey # StrFmt(Auf.P.KundenSW, 20, _StrEnd) + Lib_Strings:IntForSort(Auf.P.Nummer);
   vSortKey # AInt(CnvIa(Lib_Strings:DateForSort(Auf.P.Termin1Wunsch)));
...
//debug(Aint(Auf.P.nummer) + '/' + Aint(Auf.P.Position) + ' ' + StrFmt(Auf.P.KundenSW, 20, _StrEnd) + ' ' + CnvAd(Auf.P.Termin1Wunsch));
