...
//===== Business-Control =================================================
//
//  Prozedur    F_STD_REK_Ablehnung
//                      OHNE E_R_G
//  Info
//    Druckt eine Ablehnung
//
//
//  14.06.2008  DS  Erstellung der Prozedur
...
begin
  ...
  RekRestore(vBuf100);
  RETURN CnvAI(Rek.Nummer,_FmtNumNoGroup | _FmtNumLeadZero,0,8)+'.2'; // Dokumentennummer
end;
...
begin
  ...
  While (vGruppe=Rek.P.Referenznr) and (Rek.P.Referenznr <> '') do begin
  ...
  */
  ...
  //Lieferschein, Gesamtliefermenge
  ...
  if (Rek.P.Rechnungsnr<>0) then begin
    ...
    PL_Print(cnvAI(Rek.P.Rechnungsnr,_FmtNumNoGroup)+' vom '+cnvAD(Erl.Rechnungsdatum,_FmtInternal), cPos2a);
    PL_PrintLine;
  end;
  ...
  vGruppe # Rek.P.Referenznr;
  While (vGruppe=REk.P.Referenznr) and (Rek.P.Referenznr <> '') and (Erx <= _rLocked) do begin
    vVerp # ''
  ...
  */
  ...
  PL_Print('Ablehnungsgrund:', cPos2);
  PL_PrintLine;
  PL_PrintLine;

  pls_Fontattr # 0;
  // Reklamations-Positiontext drucken
...
begin
  ...
  Pl_Print('Ablehnung' + ' ' + AInt(Rek.P.Nummer)   ,cPos0 );
  pl_PrintLine;
  pl_PrintLine;
  ...
  if (aSeite=1) then begin
    ...
    PL_PrintLine;
    PL_Print('Die Überprüfung Ihrer Beanstandung hat zu folgendem Ergebnis geführt:',cPos0);
    PL_PrintLine;
    PL_PrintLine;
    PL_PrintLine;
...
begin
  ...
  Erx # RecLink(300, 301, 1, _recFirst)    // Kopf holen
  if(Erx > _rLocked) then
    RecBufClear(300);

  Erx # RecLink(401, 300, 4, _recFirst)   // Auftragspos holen
  if(Erx > _rLocked) then begin
    Erx # RecLink(411, 300, 6, _recFirst); // ggf. Ablage
    if(Erx > _rLocked) then
      RecBufClear(411);
  ...
  Erx # RecLink(100, 300, 9, _recFirst);   // Kunde holen
  if(Erx > _rLocked) then
    RecBufClear(100);
  ...
  Erx # RecLink(814, 300, 8, _RecFirst);   // Währung holen
  if(Erx > _rLocked) then
    RecBufClear(814);
  ...
  Erx # RecLink(301, 300, 11, _recFirst); // Positionen loopen
  WHILE (Erx <= _rLocked ) DO BEGIN

    if ("Rek.P.Löschmarker"='*') then begin    // keine geloeschten
      Erx # RecLink(301, 300, 11, _recNext);
      CYCLE;
    end;
    ...
    vSumStk       # vSumStk     + "Rek.P.Stückzahl";
    vSumGewicht   # vSumGewicht + Rek.P.Gewicht;
  ...
  pls_FontSize  # 10;
  //PL_Print('Nach Prüfung Ihrer Beanstandung erhalten Sie von uns eine Stellungnahme.',cPos1);
  //PL_PrintLine;
  //PL_PrintLine;
  PL_PrintLine;
  PL_PrintLine;
