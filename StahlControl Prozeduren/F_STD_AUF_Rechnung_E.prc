...
begin
  ...
  Pls_fontSize # 9;
  PL_Print('Supplier-No.:',cPosKopf1);
  ...
  if (Auf.Vorgangstyp=c_BOGUT) then
    Pl_Print(translate('Bonus-Credit note') + ' ' + AInt(Erl.Rechnungsnr),cPos1)
  else if (Auf.Vorgangstyp=c_REKOR) or (Auf.Vorgangstyp=c_GUT) then
    Pl_Print(translate('Credit note') + ' ' + AInt(Erl.Rechnungsnr),cPos1)
  else if (Auf.Vorgangstyp=c_BEL_KD) or (Auf.Vorgangstyp=c_BEL_LF) then
    PL_Print(translate('Debit note') + ' ' + AInt(Erl.Rechnungsnr) ,cPos1)
  else
    Pl_Print(translate('Invoice') + ' ' + AInt(Erl.Rechnungsnr) ,cPos1);

  pl_PrintLine;

  Pls_FontSize # 7;
  pls_Fontattr # 0;
  Pl_Print(translate('Rechnungsdatum ist Leistungsdatum')+translate(', soweit in Position nicht anders angegeben') ,cPos1);
  pl_PrintLine;
  ...
  if (Form_Mode<>'FUSS') then begin
    ...
    PL_Print_R('Total in'+"Wae.Kürzel",cPos7);
...
// ------- POSITIONEN --------------------------------------------------------------------------
  ...
  WHILE (Erx <= _rLocked ) DO BEGIN
    ...
    if (vTxtName != '') then
      Lib_Print:Print_Text(vTxtName,1);  // drucken
    ...
    WHILE (Erx<=_rLocked) do begin
    /*
    if (Auf.Z.MengenbezugYN) and ((Auf.Z.MEH='%') or (Auf.Z.MEH=Auf.P.MEH.Preis)) then begin
        //if ("Auf.Z.Schlüssel" = '*RAB1') or ("Auf.Z.Schlüssel" = '*RAB2') then
        //  Auf.Z.Bezeichnung # cnvAF(-Auf.Z.Menge) + ' %';
        //if (vRb1='') then vRb1 # Auf.Z.Bezeichnung;
        end
...
//      vPosMenge # Lib_Einheiten:WandleMEH(401, vPosStk, vPosGewicht, vPosMenge, Auf.P.MEH, Auf.P.MEH.Preis);
    end;
    ...
    if (Wgr_Data:IstMat(Auf.P.Wgr.Dateinr)) or (Wgr_Data:IstMix(Auf.P.Wgr.Dateinr)) then begin
      F_STD_AUF_AufBest_E:MaterialDruck(vRb1,vPosMengeLFS, vPosMenge, vPosStk);
    end;
...
debug('HONK HONK');
    ...
    */
    if (Wgr_Data:IstArt(Auf.P.Wgr.Dateinr)) and
      ...
      WHILE (Erx<=_rLocked) do begin
        if (Auf.P.Artikelnr=Auf.SL.Artikelnr) then begin
          PL_Print(cnvAI("Auf.SL.Stückzahl")+' a',cPos2+5.0);
          PL_Print_R(cnvAF("Auf.SL.Länge",0,0,0,6) + ' mm',cPos2+30.0);
          end
        else begin
          if ("Auf.SL.Länge"<>0.0) then begin
            PL_Print(cnvAI("Auf.SL.Stückzahl")+' Stk. von '+Auf.SL.ArtikelNr+' a '+cnvAF("Auf.SL.Länge",0,0,0,6) + ' mm',cPos2+5.0);
            end
          else begin
            PL_Print(cnvAI("Auf.SL.Stückzahl")+' Stk. von '+Auf.SL.ArtikelNr,cPos2+5.0);
          end;
        end;
        PL_PrintLine;
    ...
    if (vGibtsPosZ) then begin
      ...
      PL_PrintLine;
      pls_FontAttr # _WinFontAttrBold;
      PL_Print('zusätzliche Positionsaufpreise und Rabatte',cPos2);
      pls_FontAttr # 0;
      PL_PrintLine;
      Lib_Print:Print_LinieEinzeln(cPos2,cPos7);
...
/*    else
  ...
  */
  ...
  // Summen drucken
  pls_Fontsize # 10;
  PL_Print_R('Total '+"Wae.Kürzel",cPos7-25.0);
...
//  PL_PrintLine;
  ...
  vA # Ofp_data:BuildZabString(Zab.Bezeichnung1.L2, Ofp.Skontodatum, Ofp.Zieldatum, OfP.Skontoprozent, OfP.Brutto);
  PL_Print(vA,cPosFuss2);
  PL_PrintLine;
  if (ZaB.Bezeichnung2.L2<>'') then begin
    vA # Ofp_data:BuildZabString(Zab.Bezeichnung2.L2, Ofp.Skontodatum, Ofp.Zieldatum, OfP.Skontoprozent, OfP.Brutto);
    PL_Print(vA,cPosFuss2);
    PL_PrintLine;
  end;
...
//  PL_PrintLine;
  ...
  Lib_Print:Print_Text(vTxtName, 1, cPos0, cPos7);
