...
//==== Business-Control ==================================================
//
//  Prozedur    Lfs2_P_LFA_Main
//                  OHNE E_R_X
//  Info
//
//
//  10.10.2011  AI  Erstellung der Prozedur
//  13.12.2011  AI  VSB nur löschen, wenn nicht bereits gelöscht ist
//  25.05.2016  AH  "ausWE" Nutzt "MeldeWEaufVSB"
//  06.09.2016  AH  Edit nicht erlaubt
//  18.04.2019  AH  Fix: Beim manuellen FM-Dialog führt "J" "N" nicht zum Schließen
//  02.03.2020  AH  Fertigmelden mit RekSave weiter abgesichert
//  12.05.2022  AH  ERX
//
//  Subprozeduren
...
//    SUB Fertigmelden();
//    SUB EvtKeyItemDlg...
//    SUB EvtMenuCommandDLG(aEvt : event; aMenuItem : int) : logic
//    SUB EvtKeyItemSpezi(aEvt : event; aKey : int; aID : int) : logic;
//    SUB EvtMouseItemSpezi(aEvt : event; aButton : int; aHitTest : int; aItem : int; aID : int) : logic;
//
//========================================================================
@I:Def_Global
@I:Def_Aktionen
...
//    Error(010034,AInt(BAG.P.Nummer));
  ...
  vDatum    # cFertigDatum;
  vWerksnr  # StrCut(cFertigWerksnr,1,16);
...
begin
  ...
  TRANSON;
  Lib_Misc:AddTodo('LFS_'+aint(Lfs.Nummer)+'_WE2VSB');    // 10.11.2020 AH: Merken, dass wir WE buchen um z.B. Kreditlimit anders prüfen (VFP)
  if (Lfs_LFA_Data:MeldeWEaufVSB(vMat, vErsetzen, vVSBKillen, vAutoFM)=false) then begin
    TRANSBRK;
    ErrorOutput;
    RETURN;
  end;
  Lib_Misc:DelTodo('LFS_'+aint(Lfs.Nummer)+'_WE2VSB')
  TRANSOFF;
...
/***
  ...
  if (vAutoFM) then begin
    ...
    if (Lfs_LFA_Data:Verwiegung_Mat("Lfs.P.Stück", vGewN, vGewB, 0.0, '', today, '', '')=false) then begin
      // Fehler!
//      TRANSBRK;
      Error(441701,'');
    end;
...
//                          _WinLstFromFirst | _WinLstRecDoSelect);
  ...
  // Focus setzen:
//09.03.2017 AH doppelt?  gZLList->Winfocusset(false);
end;
...
//
  ...
  if (LFS.P.Materialtyp=c_IO_VSB) then begin
    ...
    RekRestore(v441);
    RETURN;   // Abbruch wegen MDI-Start
  end;

  RecBufCopy(v441,441);


  vWin # WinOpen('Dlg.LFA.FM.Mat',_winopendialog);
  vHDL # Winsearch(vWIN,'edSTUECK');
  vHDL->wpcaptionint # 0;
  vHDL # Winsearch(vWIN,'edNETTO');
  vHDL->wpDecimals      # Set.Stellen.Gewicht;
  vHDL->wpcaptionfloat  # 0.0;
  vHDL # Winsearch(vWIN,'edBRUTTO');
  vHDL->wpDecimals      # Set.Stellen.Gewicht;
  vHDL->wpcaptionfloat # 0.0;
  if (LFs.P.MEH<>'kg') and (LFs.P.MEH<>'Stk') then begin
    vHDL # Winsearch(vWIN,'edMENGE');
    vHDL->wpDecimals      # Set.Stellen.Menge;
    vHDL->wpcaptionfloat  # 0.0;
    vHDL->wpvisible       # true;
    vHDL # Winsearch(vWIN,'lbMEH');
    vHDL->wpcaption # LFS.P.MEH;;
    vHDL->wpvisible       # true;
    vHDL # Winsearch(vWIN,'lbMENGE');
    vHDL->wpvisible       # true;
  end;
  vHDL # Winsearch(vWIN,'edDATUM');
  vHDL->wpcaptiondate # today;
  vHDL # Winsearch(vWIN,'edBEM');
  vHDL->wpcaption     # '';
  vHDL # Winsearch(vWIN,'edWERKSNUMMER');
  vHDL->wpcaption     # '';
  ...
  vHDL # Winsearch(vWIN,'edLagerplatz');
  vHDL->wpcaption     # vLager;

  vID # vWin->WinDialogRun(_WinDialogCenter, gMDI)

  vHDL # Winsearch(vWIN,'edSTUECK');
  vStk # vHDL->wpcaptionint;
  vHDL # Winsearch(vWIN,'edNETTO');
  vNetto # vHDL->wpcaptionfloat;
  vHDL # Winsearch(vWIN,'edBRUTTO');
  vBrutto # vHDL->wpcaptionfloat;
  vHDL # Winsearch(vWIN,'edMENGE');
  vMenge # vHDL->wpcaptionfloat;
  vHDL # Winsearch(vWIN,'edDATUM');
  vDatum # vHDL->wpcaptiondate;
  vHDL # Winsearch(vWIN,'edBEM');
  vBem # vHDL->wpcaption;
  vHDL # Winsearch(vWIN,'edWERKSNUMMER');
  vWerksNr # vHDL->wpcaption;
  vHDL # Winsearch(vWIN,'edLagerplatz');
  vLager # vHDL->wpcaption;
  ...
  if (vID=_WinIdOk) then begin
    ...
    RecbufCopy(v441,441);
    RecLink(440,441,1,_recFirst);   // Kopf holen

    if (Lfs_LFA_Data:Verwiegung_Mat(vStk, vNetto, vBrutto, vMenge, LFS.P.MEH, vDatum, vBem, vWerksNr, vLager)=false) then begin
    // Fehler!
      Error(441701,'');
      ErrorOutput;
      RekRestore(v441);
...
begin
  ...
  vGew    # $edNETTO->wpcaptionfloat;
  vMenge  # $edMENGE->wpcaptionfloat;

  if (aMenuItem->wpname='Mnu.Ktx.Errechnen') then begin
    if (aEvt:Obj->wpname='edMENGE') then begin
      if (Lfs.P.Gewicht.Netto<>0.0) then
        vMenge # Rnd(Lfs.P.Menge / Lfs.P.Gewicht.Netto * vGew, Set.Stellen.Menge);
      end
    else if (aEvT:Obj->wpname='edBRUTTO') or (aEvT:Obj->wpname='edNETTO') then begin
      if (Lfs.P.Menge<>0.0) then
        vGew # Rnd(Lfs.P.Gewicht.Netto / Lfs.P.Menge * vMenge, Set.Stellen.Gewicht);
    end;
  end;

  $edBRUTTO->wpcaptionfloat # vGew;
  $edNETTO->wpcaptionfloat  # vGew;
  $edMENGE->wpcaptionfloat  # vMenge;
...
//  RETURN true;    09.07.2015 AH
  RETURN App_Main:EvtKeyItem(aEvt, aKey, aID);
...
begin
  ...
  if (aID <> 0 AND aItem <> 0) and
    ((aButton & (_WinMouseLeft | _WinMouseDouble) = (_WinMouseLeft | _WinMouseDouble))) then begin
  ...
  RETURN true;
  //App_Main:EvtMouseItem(aEvt, aButton, aHitTest, aItem, aID);
