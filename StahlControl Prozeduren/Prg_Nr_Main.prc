...
//==== Business-Control ==================================================
//
//  Prozedur    Prg_Nummern_Main
//                  OHNE E_R_G
//  Info
//
//
//  19.04.2004  TM  Erstellung der Prozedur
//  16.03.2022  AH  ERX
//  2023-07-06  ST  Excelexport/Import hinzugefügt
//
//  Subprozeduren
...
begin
  ...
  $edPrg.Nr.Name->WinFocusSet(true);
end;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Nrs_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Nrs_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Nrs_Loeschen]=n);
