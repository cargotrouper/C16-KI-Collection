...
//==== Business-Control ==================================================
//
//  Prozedur    BA1_F_SpSpulen_Main
//                    OHNE E_R_G
//  Info
//
//
//  28.09.2015  AI  Erstellung der Prozedur
//  05.04.2022  AH  ERX
//  16.05.2022  AH  Umbau auf Spulenzahl und Spulenbreite
...
define begin
  cDialog :   $BA1.F.SpSpulen.Maske
  cTitle :    'Spalt-Spulen-Fertigung'
  cFile :     703
  cMenuName : 'BA1.F.Bearbeiten'
  cPrefix :   'BA1_F_SpSpulen'
  cKey :      1
end;
...
***/
  ...
  if (aName='') or (y) then begin
    ...
    $Lb.SpulBreite.E->wpcaption # ANum(BAG.IO.SpulBreite,"Set.Stellen.Breite");
    if (BAG.IO.SpulBreite<>0.0) then begin
      $Lb.SpulBreite.E->wpvisible # true;
      $Lb.SpulBreite.E2->wpvisible # true;
      $Lb.SpulBreite.E3->wpvisible # true;
    end;
...
//    BA1_F_Data:AusKommission(Auf.P.Nummer, Auf.P.Position, gSelected);
  ...
  WHILE (Erx<=_rLocked) do begin
    ...
    if (BAG.F.Fertigung<>vmyFert) then begin
      ...
      if (RecInfo(703,_recid)=vGegenID) then begin
        if (BAG.P.Aktion=c_BAG_SpaltSpulen) and (BAG.F.StreifenAnzahl=1) then
