...
begin
  if (aName='') or (aName='edWae.Krzel') then begin
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Wae_Anlegen]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Wae_aendern]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Wae_loeschen]=n);
  // Menü sperren
