...
//  23.01.2020  AH  Neu: "Excel.Export + Import"
//  13.09.2021  ST  Neu: "InstallCtxOffice"
//  04.04.2022  AH  ERX
//  2022-09-20  AH  Proj. 1326/629
//
//  Subprozeduren
//    SUB Excel.Import(aFile : int) : logic
//    SUB Excel.Export(aFile : int) : logic
...
begin
  ...
  if (Erx=_winIdYes) then begin
    if (Lib_Excel:SchreibeDatei(aFile, vName, false)=true) then
  ...
  if (Erx=_WinIdCancel) then RETURN true;
  if (Erx=_winIdYes) then begin
    if (Lib_Excel:SchreibeDatei(aFile, vName, true)=true) then
      Msg(998004,vName+'|'+cnvai(Gv.int.01),_WinIcoInformation,0,0)
    else
      Msg(999999,gv.alpha.01,_WinIcoError,0,0);
  end;
...
begin
  ...
  If (Msg(998005, vName,_WinIcoQuestion,_WinDialogYesNo,2)=_WinidYes) then begin
    if (Lib_Excel:LiesDatei(aFile, vName, n)=true) then
...
begin
  ...
  FOR Erx # RecRead(460, vSel, _recFirst); // Selektierte Fertigungen loopen
  ...
  WHILE (Erx <= _rLocked) DO BEGIN
    ...
    FOR Erx # RecLink(461, 460, 1,_recFirst); // Selektierte Fertigungen loopen
    LOOP Erx # RecLink(461, 460, 1,_recNext);
    WHILE(Erx <= _rLocked) DO BEGIN
      if(OfP.Z.Zahlungsnr = 1) then begin
        Erx # RecRead(461, 1, _recLock);
        ...
        Erx # RekReplace(461, _recUnlock, 'AUTO');
        if(Erx <> _rOK) then begin
        end;
...
begin
  if(WinDialogBox(gFrmMain,'COMMAND', Translate('Wollen Sie alle Formular auf Vorschau setzen?'),_WinIcoInformation,_WinDialogYesNo,0) = _WinIdYes) then begin
    FOR Erx # RecRead(912, 1, _recFirst | _recLock);
    LOOP Erx # RecRead(912, 1, _recNext | _recLock);
    WHILE(Erx = _rOK) DO BEGIN
      Frm.DirektdruckYN # false;
      Frm.VorschauYN # true;
      Erx # RekReplace(912, _recUnlock, 'AUTO');
    END;
...
begin
  ...
  vToken # StrCnv((StrAdj(StrCut(aCmd, StrFind(aCmd, ' ', 0) + 1, Erx - (StrFind(aCmd, ' ', 0) + 1)), _StrBegin | _StrEnd)), _StrUpper);
  vNewToken # StrCnv(StrAdj(StrCut(aCmd, Erx + 2, StrLen(aCmd) - Erx + 2), _StrBegin | _StrEnd), _StrUpper);
  ...
  if(WinDialogBox(gFrmMain,'COMMAND', Translate('Wollen Sie Prozeduren mit Namen ' + vToken + ' kopieren und in ' + vNewToken + ' umbennen?'),_WinIcoInformation,_WinDialogYesNo,0) = _WinIdYes) then begin
    if(WinDialogBox(gFrmMain,'COMMAND', Translate('SICHER?'),_WinIcoInformation,_WinDialogYesNo,0) = _WinIdYes) then begin
      ...
      LOOP Erx # vHdl -> TextRead(vProc, _TextNext | _TextProc);
      WHILE(Erx <= _rLocked) DO BEGIN
...
begin
  ...
  WHILE(Erx <= _rLocked) DO BEGIN
    ...
    Erx # RecRead(501, 1, _recLock); // Position sperren
    if(Erx <> _rOK) then begin
      TRANSBRK;
    ...
    Erx # Ein_Data:PosReplace(_recUnlock, 'MAN'); // Position zurueckspeichern
    if(Erx <> _rOK) then begin
      TRANSBRK;
...
begin
  ...
  LOOP Erx # RecRead(501, 1, _recNext)
  WHILE(Erx <= _rLocked) DO BEGIN
    Erx # RecLink(504, 501, 15, _recFirst); // 1 Aktion holen
...
begin
  ...
  WHILE(Erx <= _rLocked) DO BEGIN
    Erx # RecRead(401, 1, _recLock); // Position sperren
    if(Erx <> _rOK) then begin
      TRANSBRK;
    ...
    Erx # Auf_Data:PosReplace( _recUnlock, 'MAN'); // Position zurueckspeichern
    if(Erx <> _rOK) then begin
      TRANSBRK;
  ...
  if(ErrList = 0) then begin
    ...
    WHILE(Erx <= _rLocked) DO BEGIN
      Erx # RecRead(411, 1, _recLock); // Position sperren
      if(Erx <> _rOK) then begin
        TRANSBRK;
        Error(001001, AInt("Auf~P.Nummer") + '/' + AInt("Auf~P.Position"));
        vProgress->Lib_Progress:Term();
      ...
      Erx # RekReplace(411, _recUnlock, 'MAN'); // Position zurueckspeichern
      if(Erx <> _rOK) then begin
        TRANSBRK;
        Error(999999, 'SavPos ' + AInt("Auf~P.Nummer") + '/' + AInt("Auf~P.Position"));
        vProgress->Lib_Progress:Term();
...
begin
  ...
  LOOP Erx # RecRead(401, 1, _recNext)
  WHILE(Erx <= _rLocked) DO BEGIN
    Erx # RecLink(404, 401, 12, _recFirst); // 1 Aktion holen
    if(Erx > _rLocked) then
      RecBufClear(404);
...
begin
  ...
  FOR Erx # RecRead(400, vSel, _recFirst); // Selektierte Fertigungen loopen
  ...
  WHILE (Erx <= _rLocked) DO BEGIN
    ...
    Erx # RecLink(401, 400, 9, _recFirst); // 1. Auftragspos.
    if(Erx > _rLocked) then
      RecBufClear(401);
...
//      Lib_Rec:ClearFile( 406 ); // Einteilung
    ...
    if ( vHdl->WinSearch( 'cb700' )->wpCheckState = _winStateChkChecked ) and
      ...
      WHILE (Erx<=_rLocked) do begin
        if ((BAG.VorlageYN) and (vHdl->WinSearch( 'cb700Vorlage' )->wpCheckState = _winStateChkChecked )) or
          ...
          LOOP Erx # RecLink(701,700,3 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(701);
          END;
          FOR Erx # RecLink(702,700,1 ,_RecFirst)
          LOOP Erx # RecLink(702,700,1 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
          ...
          LOOP Erx # RecLink(703,700,6 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            ...
            RekDelete(703);
          END;
          FOR Erx # RecLink(704,700,2 ,_RecFirst)
          LOOP Erx # RecLink(704,700,2 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(704);
          END;
          FOR Erx # RecLink(705,700,7 ,_RecFirst)
          LOOP Erx # RecLink(705,700,7 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(705);
          END;
          FOR Erx # RecLink(706,700,8 ,_RecFirst)
          LOOP Erx # RecLink(706,700,8 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(706);
          END;
          FOR Erx # RecLink(707,700,5 ,_RecFirst)
          LOOP Erx # RecLink(707,700,5 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(707);
          END;
          FOR Erx # RecLink(708,700,11,_RecFirst)
          LOOP Erx # RecLink(708,700,11,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(708);
          END;
          FOR Erx # RecLink(709,700,9 ,_RecFirst)
          LOOP Erx # RecLink(709,700,9 ,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(709);
          END;
          FOR Erx # RecLink(710,700,12,_RecFirst)
          LOOP Erx # RecLink(710,700,12,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(710);
          END;
          FOR Erx # RecLink(711,700,10,_RecFirst)
          LOOP Erx # RecLink(711,700,10,_RecFirst)
          WHILE (Erx<=_rLocked) do begin
            RekDelete(711);
          END;
    ...
    if ( vHdl->WinSearch( 'cb570' )->wpCheckState = _winStateChkChecked ) then begin // Kassenbuch
      ...
      Lib_Rec:ClearFile( 580 ); // Kosten
      Lib_Rec:ClearFile( 581 ); // KostenPosten
    end;
    vPrgr->Lib_Progress:Step();
    ...
    /* Schlüsseldateien */
    if ( vHdl->WinSearch( 'cb830' )->wpCheckState = _winStateChkChecked ) then begin // Kalkulationen
      Lib_Rec:ClearFile( 830 ); // Kopf
      Lib_Rec:ClearFile( 831 ); // Position
    end;
    ...
    if ( vHdl->WinSearch( 'cb8xx' )->wpCheckState = _winStateChkChecked ) then begin // alle Schlüsseldateien
      ...
      Lib_Rec:ClearFile( 845 ); // Toleranzen
      Lib_Rec:ClearFile( 846 ); // Kostenstellen
      Lib_Rec:ClearFile( 848 ); // MQu Stufen
      ...
      Lib_Rec:ClearFile( 854 ); // Gegenkonten
      Lib_Rec:ClearFile( 855 ); // Zeitentypen
      Lib_Rec:ClearFile( 856 ); // Artikelzustände
    end;
    if ( vHdl->WinSearch( 'cb860' )->wpCheckState = _winStateChkChecked ) then begin // Customtabelle
      Lib_Rec:ClearFile( 860 ); // Kopf
    end;
    if ( vHdl->WinSearch( 'cb920' )->wpCheckState = _winStateChkChecked ) then begin // Skripte
      Lib_Rec:ClearFile( 920 ); // Kopf
      Lib_Rec:ClearFile( 921 ); // Position
    end;
    vPrgr->Lib_Progress:Step();
...
***/
    ...
    if ( vHdl->WinSearch( 'cb100' )->wpCheckState = _winStateChkChecked ) then begin // Adressen
      ...
      Lib_Rec:ClearFile( 104 ); // Aufpreise
    ...
    if ( vHdl->WinSearch( 'cb250' )->wpCheckState = _winStateChkChecked ) then begin // Artikel
      ...
      Lib_Rec:ClearFile( 257 ); // Ausführungen
    ...
    if ( vHdl->WinSearch( 'cb160' )->wpCheckState = _winStateChkChecked ) then begin // Ressourcen
      ...
      Lib_Rec:ClearFile( 170 ); // Rso-Reservierungen   08.03.2021 AH: Zustzälich zu BA
      Lib_Rec:ClearFile( 171 ); // Rso-Res-Verbindungen
    end;
    vPrgr->Lib_Progress:Step();
    ...
    if ( vHdl->WinSearch( 'cb922' )->wpCheckState = _winStateChkChecked ) then begin // SFX/AFX/Customfleder
      ...
      // AFX leeren...
      FOR Erx # RecRead(923,1,_recfirst)
      LOOP Erx # RecRead(923,1,_recNext)
      WHILE (Erx<=_rLocked) do begin
...
begin
//debug(aDB+ ' '+aAFX);
  Erx # DBAConnect(2,'X_','TCP:'+aServ, aDB, aUser, aPass,'');
  if (Erx<>_rOK) then RETURN 'ERROR:'+aint(Erx);

  Try begin
    ErrTryIgnore(_ErrStringOverflow);
    FldDef(2923,1,1,aAFX);
  end;
  Erx # RecRead(2923,1,0);
  if (Erx<=_rLocked) then vWert # FldAlpha(2923,1,2);
  DBADisconnect(2);
...
//  Msg(99,'Ausgabe in der DEBUG.TXT !',0,0,0)
  Erx # WindialogBox(gFrmMain,'AFX : '+aCmd,vText, _WinIcoInformation, _WinDialogOk|_WinDialogAlwaysOnTop,1);

end;


//========================================================================
// "CreateCustom Adr.Verwaltung SFX_Horst" wird SFX.Horst.Adr.Verwaltung" ODEr
// "CreateCustom Adr.Verwaltung XXX.Verwaltung YYY_Main" für "XXX.Verwaltung" und Proc. "YYY_Main"
// CreateCustom Auf.P.Verwaltung Auf.P2.Verwaltung Auf_P2_Main
//========================================================================
SUB CreateCustom(
  aDlgName      : alpha;
  aPrefix       : alpha;
...
begin
  ...
  if (aProcName<>'') then begin
    ...
    if (StrCut(aPrefix,StrLen(aPrefix), 1)<>'_') and
      (StrCut(aPrefix,StrLen(aPrefix), 1)<>'.') then
      aPrefix # aPrefix + '.';
...
//  vHdl # WinOpen(aDlgName, _WinOpeneventsoff);
  ...
  if (aProcName<>'') then
    vProcNeu # aProcName;
  if (StrLen(vProcNeu)>20) then begin
    Msg(99,'Prozedurname wäre zu lang mit '+vProcNeu,0,0,0);
    Winclose(vHdl);
