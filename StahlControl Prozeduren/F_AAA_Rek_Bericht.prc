...
//===== Business-Control =================================================
//
//  Prozedur    F_AAA_Rek_Bericht
//                      OHNE E_R_G
//  Info
//    Druckt einen Recklamationsbericht
//
//
...
begin
  ...
  WHILE (Erx<=_rLocked) do begin
    ...
    if (Rek.A.AnerkennungYN) then begin
      vAnerGesStk     # vAnerGesStk + "Rek.A.Stückzahl";
      vAnerGesGew     # vAnerGesGew + Rek.A.Gewicht;
      vAnerGesMenge   # vAnerGesMenge + Rek.A.Menge;
      vAnerGesWert    # vAnerGesWert + Rek.A.Kosten;
      vAnerGesWertW1  # vAnerGesWertW1 + Rek.A.KostenW1;
    end;
...
begin
  ...
  if (Rek.ZuDatei=400) then begin
    ...
    Ein_Data:Read(Rek.Einkaufsnr, Rek.Einkaufspos,y); // Bestellung holen
    Erx # RekLink(819,501,1,0);                       // Warengruppe holen
    RekLink(100,500,1,_RecFirst);   // Lieferant holen
    RecBufClear(400);
    RecBufClear(401);
  end;
...
// ------- POSITIONEN --------------------------------------------------------------------------
  ...
  WHILE (Erx<=_rLocked) DO BEGIN
    ...
    RecBufClear(160);
    if (Rek.P.VerursacherGrp<>0) then
      RekLink(160,301,9,_recFirst);                   // Ressource holen

    RekLink(851,301,8,_recFirst);                     // Fehlercode holen
    RekLink(850,301,7,_recFirst);                     // Status holen
    ...
    // Material ausgeben
    if (Wgr_Data:IstMix()) or (Wgr_Data:IstMat()) then begin
    ...
    // Material ausgeben
    if (Wgr_Data:IstMat()) or (Wgr_Data:IstMix()) then
...
//    form_Ele_Rek:elPosTextUS(var elPosTextUS);
  ...
  Rek.P.Aner.Menge    # vAnerGesMenge;
  Rek.P.Aner.Stk      # vAnerGesStk;
  Rek.P.Aner.Gew      # vAnerGesGew;
  Rek.P.Aner.Wert     # vAnerGesWert;
  Rek.P.Aner.WertW1   # vAnerGesWertW1;
...
//    form_Elemente:Leerzeile(var elLeerzeile);
  ...
  // Summe vorbelegen
  form_Ele_Rek:elSumme(var elSumme);
