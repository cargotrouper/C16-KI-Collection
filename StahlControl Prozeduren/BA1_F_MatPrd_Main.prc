...
define begin
  cDialog :   $BA1.F.MatPrd.Maske
  cTitle :    'Material-Produktion Fertigung'
  cFile :     703
  cMenuName : 'BA1.F.Bearbeiten'
  cPrefix :   'BA1_F_MatPrd'
  cKey :      1
end;
...
begin
  ...
  Lib_GuiCom:Pflichtfeld($edBAG.F.Stueckzahl);
...
begin
  ...
  if (mode=c_ModeEdit) and ((BAG.F.AutomatischYN) and (Bag.F.Fertigung=999)) then begin
    ...
    Lib_GuiCom:Disable($edBAG.F.MEH);
...
begin
  ...
  If (BAG.F.Warengruppe=0) then begin
    Lib_Guicom2:InhaltFehlt('Warengruppe', 'NB.Page1', 'edBAG.F.Warengruppe');
    RETURN false;
  end;
  Erx # RecLink(819,703,5,_recFirst);   // Warengruppe holen
  if (Erx>_rLocked) then begin
    Lib_Guicom2:InhaltFalsch('Warengruppe', 'NB.Page1', 'edBAG.F.Warengruppe');
    RETURN false;
  end;

  if (Wgr_Data:IstMix()) and (BAG.F.Artikelnummer='') and (BAG.VorlageYN=n) then begin
    Lib_Guicom2:InhaltFehlt('Artikelnummer', 'NB.Page1', 'edBAG.F.Artikelnummer');
    RETURN false;
  end;
  if (Wgr_Data:IstMix()) and (BAG.F.Artikelnummer<>'') then begin
    Erx # RekLink(250,703,13,_recFirst);    // Artikel holen
    if (Erx>_rLocked) then begin
      Lib_Guicom2:InhaltFalsch('Artikelnummer', 'NB.Page1', 'edBAG.F.Artikelnummer');
      RETURN false;
    end;
  end;
  ...
  if (BAG.F.ReservierenYN) then begin
    if ("BAG.F.ReservFürKunde"=0) then begin
      Lib_Guicom2:InhaltFalsch('Kunde', 'NB.Page1', 'edBAG.F.ReservFuerKunde');
      RETURN false;
    end;
    Erx # RecLink(100,703,7,_recFirsT);   // Kunde holen
    if (Erx>_rLocked) then begin
      Lib_Guicom2:InhaltFehlt('Kunde', 'NB.Page1', 'edBAG.F.ReservFuerKunde');
      RETURN false;
    end;
  end;

  If ("BAG.F.Stückzahl"<=0) then begin
    Lib_Guicom2:InhaltFehlt('Stückzahl', 'NB.Page1', 'edBAG.F.Stueckzahl');
    RETURN false;
  end;
