...
//==== Business-Control ==================================================
//
//  Prozedur    Art_Zst_Main
//                  OHNE E_R_G
//  Info
//
//
//  26.07.2010  AI  Erstellung der Prozedur
//  06.01.2014  ST  "cListen" Angabe hinzugefügt
//  04.04.2022  AH  ERX
//
//  Subprozeduren
...
define begin
  cTitle      : 'Artikel-Zustände'
  cFile       :  856
  cMenuName   : 'Std.Bearbeiten'
  cPrefix     : 'Art_Zst'
  cZList      : $ZL.Art.Zustaende
  cKey        : 1
  cListen     : 'Artikelzustände'
end;
...
begin
  ...
  $edArt.Zst.Nummer->WinFocusSet(true);
end;
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Art_Zst_Anlegen]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Art_Zst_Aendern]=n);
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_Art_Zst_Loeschen]=n);
