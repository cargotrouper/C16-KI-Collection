...
begin
  ...
  WHILE vBAGP  <> '' DO BEGIN
    ...
    if (aLohngeschaft = false) then begin
      ...
      if (vEinsatzGewProPos <> 0.0) then begin
        vKostenProT # vKostenProPos / vEinsatzGewProPos;
        vMatKosten # vMatKosten + (aGew * vKostenProT);
      end;
    end else begin
...
begin
  case aName of
    ...
    'HEADER1' : begin
      ...
      List_Spacing[12]  # 25.0; // MAt EK GEsamt
      List_Spacing[13]  # 25.0; // Mat Mat Frachten Gesamt
      List_Spacing[14]  # 25.0; // Mat Kosten Gesamt
      List_Spacing[15]  # 25.0; // Mat Prod Kosten Gesamt
      List_Spacing[16]  # 25.0; // Mat Schrott

      List_Spacing[17]  # 25.0; // Roh/t ohne Fracht
      List_Spacing[18]  # 25.0; // Roh/t mit Fracht
      List_Spacing[19]  # 25.0; // Fracht / t
      List_Spacing[20]  # 25.0; // Rohgesamt
      List_Spacing[21]  # 25.0; // Prodk./t
      List_Spacing[22]  # 25.0; // DB2 / t
      List_Spacing[23]  # 25.0; // DB2
      List_Spacing[24]  # 25.0; // Rechnungstyp
      List_Spacing[25]  #  0.1; // Ende
      Lib_List2:ConvertWidthsToSpacings( 25 ); // Spaltenbreiten konvertieren
      ...
      LF_Set( 8, 'Dicke'             ,y , 0);
      LF_Set( 9, 'Breite'            ,y , 0);
      LF_Set(10, 'Re Gewicht kg'     ,y , 0);
      LF_Set(11, 'Erl Netto'          ,y , 0);
      LF_Set(12, 'Mat EK Gesamt'      ,y , 0);
      LF_Set(13, 'Mat Frachten Gesamt',y , 0);
      LF_Set(14, 'Mat KostenFM Gesamt',y , 0);
      LF_Set(15, 'Mat Prod Kst Gesamt',y , 0);
      LF_Set(16, 'Mat Schrott kg'     ,y , 0);

      LF_Set(17, 'Roh/t ohne Fracht' ,y , 0);
      LF_Set(18, 'Roh/t mit Fracht'  ,y , 0);
      LF_Set(19, 'Fracht / t'        ,y , 0);
      LF_Set(20, 'Rohgesamt'         ,y , 0);
      LF_Set(21, 'Prodk./t'          ,y , 0);
      LF_Set(22, 'DB2 / t'           ,y , 0);
      LF_Set(23, 'DB2'               ,y , 0);
      LF_Set(24, 'ReTyp'             ,y , 0);
    end;
    ...
    'AUFTRAG' : begin
      ...
      if (aPrint) then begin
        ...
        Lf_Text(12,Anum(gvMatsEK    , 2));
        Lf_Text(13,Anum(gvMatsFracht, 2));
        Lf_Text(14,Anum(gvMatsKosten, 2));
        Lf_Text(15,Anum(gvProdKosten, 2));
        Lf_Text(16,Anum(gvMatSchrott, 2));
        Lf_Text(17,Anum(gvRohTOhneFracht ,2));
        Lf_Text(18,Anum(gvRohTMitFracht  ,2));
        Lf_Text(19,Anum(gvFrachtKosten  ,2));
        Lf_Text(20,Anum(gvRohGesamt      ,2));
        Lf_Text(21,Anum(gvProdKosten     ,2));
        Lf_Text(22,Anum(gvDB2t           ,2));
        Lf_Text(23,Anum(gvDB2            ,2));
        RETURN;
      end;
      ...
      LF_Set(12, '#EK-Kosten'           ,y , _LF_Wae);
      LF_Set(13, '#EK-Frachten'         ,y , _LF_Wae);
      LF_Set(14, '#Mat-Kosten'          ,y , _LF_Wae);
      LF_Set(15, '#Prod-Kosten'         ,y , _LF_Wae);
      LF_Set(16, '#Schrottgew'          ,y , _LF_Wae);

      LF_Set(17, '#Roh/t ohne Fracht'   ,y , _LF_Wae);  // cPosRohTOhneFracht # Ums.A.Einzelpreis - vEK - vFracht;
      LF_Set(18, '#Roh/t mit Fracht'    ,y , _LF_Wae);  // cPosRohTMitFracht # Ums.A.Einzelpreis - vEK;
      LF_Set(19, '#Fracht/t'            ,y , _LF_Wae);  // alt: vFracht auf Vorkalkulation / neu: EchteFrachtkosten
      LF_Set(20, '#Rohgesamt'           ,y , _LF_Wae);  // cPosRohGesamt # cPosRohTOhneFracht * (Ums.A.Gewicht.Brutto / 1000);
      LF_Set(21, '#Prodk./t'            ,y , _LF_Wae);  // vRechPrdKostGes
      LF_Set(22, '#DB2 / t'             ,y , _LF_Wae);  // cPosDB2T  #  Fix(cPosDB2 / (Ums.A.Gewicht.Brutto / 1000),2);
      LF_Set(23, '#DB2'                 ,y , _LF_Wae);  // cPosDB2  # cPosRohGesamt - vRechPrdKostGes;
      LF_Set(24, '@Erl.Rechnungstyp'    ,y , _LF_IntNG);
...
*/
    ...
    'SUM_KUNDE' : begin
      if (aPrint) then begin
        ...
        LF_Sum(20, cSum_RohGes_Kunde , 2);
        LF_Sum(23, cSum_DB2_Kunde, 2);
        RETURN;
      end;
      ...
      LF_Set(18, '' ,y , 0);
      LF_Set(19, '' ,y , 0);
      LF_Set(20, '#RohGesamt'  ,y , _LF_Wae);
      LF_Set(21, '' ,y , 0);
      LF_Set(22, '' ,y , 0);
      LF_Set(23, '#DB2'        ,y , _LF_Wae);
      LF_Set(24, '' ,y , 0);
    end;
    ...
    'SUM_WGR' : begin
      if (aPrint) then begin
        ...
        LF_Sum(20, cSum_RohGes_Warengr , 2);
        LF_Sum(23, cSum_DB2_Warengr, 2);
        RETURN;
      end;
    ...
    'FIX_SUM' : begin
      if (aPrint) then begin
        ...
        Lf_Text(23, ANum(GetSum(cSum_DB2_Gesamt) - GetSum(cSum_Fix_Gesamt),2)) ;
        RETURN;
      end;
    ...
    'SUM_GESAMT' : begin
      if (aPrint) then begin
        ...
        LF_Sum(20, cSum_RohGes_Gesamt, 2);
        LF_Sum(23, cSum_DB2_Gesamt, 2);
        RETURN;
      end;
...
local begin
  ...
  vEinsatzLautSchrottumlage : float;
  vDat1,vDat2 : date;
end;
begin
...
/*
  vQ450b # '';
  Lib_Sel:QInt(var vQ450b, 'Erl.StornoRechNr', '<>' , 0);
  Lib_Sel:QInt(var vQ450b, 'Erl.Rechnungstyp',  '=' , 400);
  vQ451   #  vQ451 + ' AND NOT('+vQ450b+')';
*/
...
//    gvFrachtKosten  # 0.0;
    ...
    WHILE Erg = _rOK DO BEGIN
      ...
      if (Mat.Nummer = 5400) then
        debug('');


      v200 # RekSave(200);

      vEinsatzLautSchrottumlage # 0.0;
      // ------------------------------------------------
      // Schrottumlage ermitteln
      // ------------------------------------------------
      
      REPEAT
        FOR  Erg # RecLink(204,200,14,_recFirst)
        LOOP Erg # RecLink(204,200,14,_recNext)
        WHILE (erg<=_rLocked) do begin
       
          if (Mat.A.Aktionstyp=c_Akt_BA_UmlageMINUS) or (Mat.A.Aktionstyp=c_Akt_BA_UmlagePLUS) then begin

            /// BestandsGew * (Gew/t * Schrottkosten/t)
            ///-----------------------------------------
            //      EK Wert

            if (Mat.A.KostenW1 <> 0.0) AND (v200->Mat.EK.Preis <> 0.0) AND (v200->Mat.Bestand.Gew <> 0.0) then begin
              vEinsatzLautSchrottumlage # vEinsatzLautSchrottumlage  +
                  (v200->Mat.Bestand.Gew * (v200->Mat.Bestand.Gew/1000.0*Mat.A.KostenW1)) / (v200->Mat.Bestand.Gew/1000.0 * v200->Mat.EK.Preis);
            end;
            BREAK;
      ...
      // ------------------------------------------------
      // Bezugsfrachten aus Material ermitteln
      // ------------------------------------------------

      Mat_Data:Read(v200->Mat.Ursprung);
      ...
      vMatT         # Mat.Bestand.Gew/1000.0;
      gvMatsEK      # gvMatsEK     + (vMatT * (Mat.EK.Preis - vMatFracht));
      gvMatsFracht  # gvMatsFracht + (vMatT * vMatFracht);
      gvMatsKosten  # gvMatsKosten + (vMatT * Mat.Kosten);
      gvMatSchrott  # gvMatSchrott + vEinsatzLautSchrottumlage;
      ...
      if ("Mat.Vorgänger" <> Mat.Ursprung) then    // MK + TM Fix für Rechnungdruck 3002866 beinhaltete BA 4440
    ...
    // ----------------------------------------------
    // Bei Gutschriften werden die Gewicht nicht immer gefüllt
    if (Erl.Rechnungstyp = 410) AND (Erl.K.Menge <> 0.0) and (Erl.K.MEH = 'kg') AND (Erl.K.Gewicht = 0.0) then
      Erl.K.Gewicht  # Erl.K.Menge;

    // ST 2019-07-24 Bei Bonusgutschriften kein Gewicht darstellen, da diese Mengen
    //               nur "virtuell" als Berechnugsgrundlage angegeben werden, und keinen
    //               realen Bestand wiederspiegeln.
    ...
    if (Erl.K.Gewicht <> 0.0) then begin
      ...
      gvRohTOhneFracht # (Erl.K.BetragW1 - Erl.K.EKPreisSummeW1 - Erl.K.InterneKostW1 + gvMatsFracht) / (Erl.K.Gewicht/1000.0);
      gvRohTMitFracht  # (Erl.K.BetragW1 - Erl.K.InterneKostW1 - Erl.K.EKPreisSummeW1) / (Erl.K.Gewicht/1000.0);

      gvRohGesamt      # gvRohTOhneFracht * (Erl.K.Gewicht / 1000.0);
      gvProdKosten     # gvProdKosten / (Erl.K.Gewicht/1000.0);
      gvFrachtKosten   # gvMatsFracht / (Erl.K.Gewicht/1000.0);
