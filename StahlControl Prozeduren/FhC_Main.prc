...
//
//
//  29.05.2008  DS  Erstellung der Prozedur
...
begin
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_FhC_Anlegen]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_FhC_Aendern]=n);
  // Menü sperren
  ...
  if (vHdl <> 0) then
    vHdl->wpDisabled # (vHdl->wpDisabled) or (Rechte[Rgt_FhC_Loeschen]=n);
  // Menü sperren
