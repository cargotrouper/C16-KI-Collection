...
//  15.04.2013  TM  FertigungsZusatz erweitert
//
//  Subprozeduren
...
begin
  ...
  if (aHdl=0) then begin
    ...
    if (UseStyle('FertZusatz_1A')) then
      DynA('aaa');
    if (UseStyle('FertZusatz_1C')) then
      DynA('bbb');
  ...
  if (isVisible('FertZusatz_1A')) then begin
    ...
    if (isVisible('FertZusatz_1C')) then begin
    Form_Parse_BAG:Parse703Multi(var vLabels, var vInhalt, var vZusatz, 'Fertzusatz_1C', true);
    FillDynA('bbb',vLabels);
  end;
  EndPrint;
