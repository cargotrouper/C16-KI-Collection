...
//===== Business-Control =================================================
//
//  Prozedur    F_STD_BAG_LOA
//                    OHNE E_R_G
//  Info
//    Druckt einen Lohnoberflaechenbearbeitungsauftrag aus
//
//
//  17.10.2006  AI  Erstellung der Prozedur
//  17.07.2007  ST  Designumstellung
//  25.03.2008  MS  Anpassung fuer die Oberflaechenbearbeitung
...
begin
  ...
  if (aSeite=1) then begin
    ...
    PL_Print('Lohnoberflächenbearbeitungsauftrag'+' '+AInt(Bag.P.Nummer)+ '/' + AInt(Bag.P.Position)   ,cPos0 );
    pl_PrintLine;
  ...
  WHILE (RecLink(703,702,4,vFlag) <> _rNoRec) DO BEGIN
    ...
    if (BAG.F.AusfOben <> '') then begin
        ...
        WHILE (RecLink(705,703,8,vFlag) <= _rLocked ) DO BEGIN
          vFlag # _RecNext;
          if (BAG.AF.Seite = '1') and (BAG.AF.Fertigmeldung=0) then begin
            if (vVerp = '') then
    ...
    if (BAG.F.AusfUnten <> '') then begin
      ...
      WHILE (RecLink(705,703,8,vFlag) <= _rLocked ) DO BEGIN
        vFlag # _RecNext;
        if(BAG.AF.Seite = '2') and (BAG.AF.Fertigmeldung=0) then
          if(vMerker='')then
...
// ------- FUßDATEN --------------------------------------------------------------------------
  ...
  if (BAG.P.Kosten.Fix <> 0.0) or (BAG.P.Kosten.Pro <> 0.0)  then begin
    ...
    if (BAG.P.Kosten.Fix <> 0.0) then begin
      ...
      if (RecRead(814,1,0) = _rNoRec) then RecBufClear(814);
