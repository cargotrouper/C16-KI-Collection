...
//===== Business-Control =================================================
//
//  Prozedur    F_STD_REK_Bericht
//                  OHNE E_R_G
//  Info
//    Druckt einen Beanstandungsbericht
//
//
//  24.06.2008  DS  Erstellung der Prozedur
//  03.08.2012  ST  Erweiterung: Arg Dateiname für auto Dateierstellung
//  04. integriert  Abbruch der Druckerausgabe/Vorschau integriert
//  09.06.2022  AH  ERX
//
//  Subprozeduren
...
//  ADD_VERP(a,b)   : begin if (vVerp <> '') then vVerp # vVerp + ', ' + a + StrAdj(b,_StrBegin | _StrEnd); else vVerp # vVerp + a + StrAdj(b,_StrBegin | _StrEnd); end;
  ...
  cPos2   :  20.0   // Bez.
  cPos2a  :  50.0   // Werte
  cPos2c  :  70.0   // Toleranzen
  cPos2d  :  80.0   // Werte der Toleranzen
  cPos5   : 120.0   // Stückzahl aus Aktion
  cPos5a  : 122.0   // Einheit Stk
  cPos8   : 155.0   // Stück aus Position
...
begin
  ...
  RekRestore(vBuf100);
  RETURN CnvAI(Rek.Nummer,_FmtNumNoGroup | _FmtNumLeadZero,0,8)+'.3'; // Dokumentennummer
end;
...
begin
  ...
  // -- Positionsdaten --
  PL_Print(Cnvai(Rek.P.Position),cPos1);
...
begin
  ...
  PL_Print(AInt(Auf.P.Nummer) + '/' + AInt(Auf.P.Position),cPos2a);
  ...
  PL_Print(Auf.Best.Nummer,cPos2a);
