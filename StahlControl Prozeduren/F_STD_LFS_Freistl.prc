...
//===== Business-Control =================================================
//
//  Prozedur    F_STD_LFS_Freistl
//                      OHNE E_R_G
//  Info
//    Druckt einen Freistellungsauftrag aus
//
//
...
begin
  vBuf100 # RekSave(100);
  RecLink(441,440,4,_RecFirst);
  RecLink(200,441,4,_RecFirst);
  RecLink(101,200,6,_recFirst);
  RecLink(100,101,1,_recFirst);
  aAdr      # Adr.Nummer;
  aSprache  # Adr.Sprache;
  RekRestore(vBuf100);
  RETURN CnvAI(Lfs.P.Nummer,_FmtNumNoGroup | _FmtNumLeadZero,0,8);      // Dokumentennummer
end;
...
begin
  ...
  if (aSeite=1) then begin
    ...
    Pl_Print('Freistellung '+AInt(Lfs.Nummer),cPos0 );
...
// ------- POSITIONSDATEN ------------------------------------------------------------------
  ...
  WHILE (RecLink(441,440,4,vFlag) <= _rLocked) DO BEGIN
    ...
    if (Lfs.P.Materialtyp = c_IO_Mat) then begin
      ...
      // Gewicht Brutto
      PL_Printf("Mat.Gewicht.Netto",0,cPos6);

      // Gewicht Netto
      PL_Printf("Mat.Gewicht.Brutto",0,cPos7);
