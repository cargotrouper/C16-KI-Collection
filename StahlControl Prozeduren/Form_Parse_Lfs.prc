...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Lfs_E:Parse440(var aLabels, var aInhalt, var aZusatz,aText,aKombi,aSpediAdr, aStartAS);
    RETURN(vZeilen);
  end;
...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Lfs_E:Parse440Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi,aSpediAdr,aStartAS);
    RETURN(vZeilen);
  end;
...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Lfs_E:Parse441(var aLabels, var aInhalt, var aZusatz,aText,aKombi);
    RETURN(vZeilen);
  end;
  // Sonderbefehl?
  if (StrCut(aText,1,2)='my') then begin
  ...
  case (StrCnv(vFeld, _StrUpper)) of
    ...
    'GGFSTÜCK' :     if ("Lfs.P.Stück"<>0) and (Lfs.P.MEH<>'pcs') and (Lfs.P.Meh<>'kg') and(Lfs.P.Meh<>'t') then
                        vAdd # aint("Lfs.P.Stück");
...
begin
  ...
  if Form_DokSprache = 'E' then begin
    vZeilen # Form_Parse_Lfs_E:Parse441Multi(var aLabels, var aInhalt, var aZusatz,aObjName,aKombi);
    RETURN(vZeilen);
  end;
