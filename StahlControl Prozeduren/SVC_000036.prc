...
//
//  Info
//  OBF_READ: Lesen von Oberflächen und Rückgabe der angegeben Felder
//
...
begin
  ...
  vNode->apiSetDesc('Eindeutige Nummer der Oberfläche','10');
  ...
  // Haupdaten
  addToApi('Obf.Nummer','');
  addToApi('Obf.Bezeichnung.L1','');
  addToApi('Obf.Bezeichnung.L2','');
  addToApi('Obf.Bezeichnung.L3','');
  addToApi('Obf.Bezeichnung.L4','');
  addToApi('Obf.Bezeichnung.L5','');
  addToApi('Obf.Kuerzel','');
  addToApi('Obf.Gegenteil.ObfNr','');
...
begin
  ...
  if (RecRead(c_DATEI,1,0) <> _rOK) then begin
      // nicht gefunden
      aResponse->addErrNode(errSVL_Allgemein,'Oberfläche nicht gefunden');
      RETURN errPrevent;
  end;
...
*/
    ...
    'INDI' : begin
      ...
      WHILE (vNode > 0) do begin
        ...
        if (StrCut(vFldName,1,4) = 'OBF.') then begin
          ...
          if (CnvIA(aArgs->getValue(vFldName)) = 1) then begin
            ...
            CASE (toUpper(vFldName)) OF
              'OBF.KUERZEL'    : vFldName #  'Obf.Kürzel';     // TDS 1: Hauptdaten
            END;
